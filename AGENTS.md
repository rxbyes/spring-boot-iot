...
`2026-04-16` 起，协议安全运行态已切到 `published snapshot -> YAML fallback`：`iot_protocol_family_definition_snapshot / iot_protocol_decrypt_profile_snapshot` 中的 `PUBLISHED` 快照会优先重建 family 与 decrypt profile，`family:<familyCode>` 校验和 legacy `$dp` decrypt resolver 也统一复用该口径；`iot_vendor_metric_mapping_rule_snapshot` 同时成为正式运行态真相，只要产品存在任意 `PUBLISHED` 映射快照，运行时与命中预览就只按快照判定，只有产品尚无正式快照时才回退草稿表。`2026-04-17` 起，`/protocol-governance` 同页又补齐协议模板草稿、模板回放与模板快照直接发布，模板编码按 `templateCode` 业务键保存，发布动作不再伪造独立审批链；`iot_protocol_template_definition(_snapshot)` 的 schema/render、`sql/init.sql` 与 runtime bootstrap 真相已同步回写，当前仅剩共享 `dev` 环境协议治理页面验收尚未宣称完成。
