...
`2026-04-25` 起，可观测证据链第一期新增 `sys_business_event_log` 与 `sys_observability_span_log` 两张持久化证据表：`/api/*` 会沉淀 HTTP 调用片段，登录与非 GET 业务动作会沉淀业务事件；设备上行 Pipeline 在 Redis `message-flow` 之外会保留持久化调用片段摘要；调度任务运行台账与慢 SQL 也会进入统一可检索证据链。后端读侧已开放 `/api/system/observability/business-events/page`、`/api/system/observability/spans/page`、`/api/system/observability/scheduled-tasks/page`、`/api/system/observability/message-archive-batches/page`、`/api/system/observability/message-archive-batches/overview`、`/api/system/observability/message-archive-batches/compare`、`/api/system/observability/message-archive-batches/report-preview`、`/api/system/observability/spans/slow-summary`、`/api/system/observability/spans/slow-trends` 与 `/api/system/observability/trace/{traceId}` 作为证据包入口和慢点/调度热点入口；调度台账会按每次 `@Scheduled` 执行沉淀 `taskCode / triggerType / triggerExpression / duration / status / traceId`，归档批次台账分页则用于核查 `iot_message_log_archive_batch` 的确认报告、归档行数、删除行数与失败原因，overview 接口会汇总最近时间窗内的异常批次数、执行偏差总量、剩余过期总量与最近异常批次，compare 接口会按 `batchNo` 串起 dry-run / apply 报告并统一返回 `MATCHED / DRIFTED / PARTIAL / UNAVAILABLE`，报告预览接口则按 `batchNo` 只读回显仓库 `logs/observability/` 下的 JSON 摘要与 Markdown 预览。前端 `/system-log` 的系统模式当前采用“轻量总览带 + `异常排查 / 观测热点 / 归档治理` 三张平级选项卡”的结构：顶部总览带只负责方向判断与快捷切换，不回到第二层概况卡或跨页功能墙；`异常排查` 承接异常筛选、异常台账与 `证据 / 追踪` 主工作区，`观测热点` 承接 `性能慢点 Top`、慢点下钻与 `调度任务台账`，`归档治理` 承接顶部异常摘要卡、`归档批次台账`、`批次号 / 状态 / 对比结论 / 创建日期` 筛选、`仅看异常` 开关，以及详情抽屉中的 `批次对比 + 确认报告预览`。顶部 4 个总览摘要项当前支持直接切换对应选项卡，`最近异常批次` 会在归档治理页签命中时自动打开同批次详情抽屉，未命中时只在页内提示“最近异常批次不在当前结果中，请调整时间范围后重试”，并在手工修改筛选后自动清掉过期摘要态、忽略旧请求晚到回写。页面支持按热点查看最近 `24` 小时 / `7` 天 bucket 趋势、`P95 / P99 / 错误率`，并从最近调度任务、最新慢点、归档批次或异常记录复盘同一证据链上的业务事件、调用片段、批次确认结果与合并时间线；若 apply 报告缺失会按“部分可比”降级，若某批次仅保留 JSON 报告而未生成 Markdown，也按“部分可预览”继续展示摘要。`/audit-log` 审计中心继续保持原有主列表工作台，不共享 `/system-log` 的系统模式选项卡。
同日 HTTP 业务事件字典进入 B1 轻侵入阶段：产品契约、映射规则、协议治理、设备操作和验收运行等高价值路径会优先写入稳定 `event_code`，未纳入字典的接口继续回退到通用 `module.action`。
同日又补齐 E1 可观测健康门禁脚本：`python3 scripts/generate-observability-health.py --hours=24 [--policy-path=config/automation/observability-health-policy.json] [--fail-on-breaches]` 会输出 `logs/acceptance/observability-health-*.json/.md`，按真实环境库检查 Trace 覆盖率、HTTP/MESSAGE_FLOW 留痕关联率、证据包就绪率和关键标签缺失率。
同日继续补齐 E5/F1/F2 可观测日志治理：底层执行器仍是 `python3 scripts/govern-observability-logs.py [--policy-path=config/automation/observability-log-governance-policy.json] [--apply]`，但夜间 `dry-run` 与人工确认 `apply` 现统一收口到 `node scripts/auto/run-observability-log-governance.mjs [--mode=dry-run|apply]`。新入口默认输出 `logs/observability/observability-log-governance-*.json/.md`；`apply` 必须显式携带最近 `24` 小时内的 `dry-run` 报告和匹配的 `expiredRows` 确认值，例如 `--mode=apply --confirm-report=logs/observability/observability-log-governance-<timestamp>.json --confirm-expired-rows=<dry-run expiredRows>`。通过后，`sys_observability_span_log` 与 `sys_business_event_log` 继续按默认 `30 / 90` 天保留期清理，`iot_message_log` 改为先归档到 `iot_message_log_archive`、登记 `iot_message_log_archive_batch`，再删除已归档热表行；统一入口与底层报告都会输出 `archiveBatch` 摘要，供共享环境夜间巡检、人工复核和后续治理台账串联证据链。`sys_observability_span_log.tags_json` 与 `sys_business_event_log.metadata_json` 写侧当前已统一扩展脱敏 `apiKey / accessKey / privateKey / deviceSecret / merchantKey / signatureSecret` 等敏感键，并限制字符串、对象项、数组项与最终 JSON 长度；`iot_message_log.payload` 继续保留主链路原始证据，不在写侧改写。
`2026-04-26` 起，真实环境 schema sync 还会对共享库已知的 `sys_dict.uk_dict_code_tenant` 唯一性漂移执行窄口自修复：只有当列顺序完全一致、差异仅体现在 `UNIQUE / INDEX` 时才允许先验重建；若检测到重复行或其他结构差异，脚本仍会直接中止并输出阻塞明细。同日起，消息日志旧表迁移/合并完成后，脚本还会按 registry 自动对齐 `iot_message_log` 的表注释与字段注释，清理 legacy rename 遗留的英文或空注释漂移。
同日业务事件字典进入 E2 Service 事实补证阶段：正式合同发布/回滚、映射规则发布/回滚、协议族/解密档案发布/回滚、风险指标目录发布、设备命令下发完成、接入案例验收启动完成与业务验收运行启动完成，都会继续写入 `sys_business_event_log`，补齐 `releaseBatchId / approvalOrderId / ruleId / familyCode / deviceCode / jobId` 等领域上下文。
质量工场当前已收口为 `/quality-workbench` 总览下的两条主路径：`/business-acceptance` 业务验收台面向验收人员、产品和项目经理，只暴露 `环境 / 账号模板 / 模块范围` 三类轻配置，并在结果首屏直接回答“是否通过”“哪些模块没过”；`/automation-governance` 自动化治理台只对研发、测试和管理员可见，并在同一路由内统一承接 `资产编排 / 执行配置 / 结果证据` 三个工作区。`runId` 深链当前统一使用 `/automation-governance?tab=evidence&runId=...`。质量工场当前只验证并维护这两条新路径，不再保留旧 `研发工场 / 执行中心 / 结果与基线中心` 入口或兼容跳转；细粒度按钮权限与共享环境菜单 seed 也统一收口到 `system:automation-governance:*`，旧 quality 工场菜单码必须同步退场。
`2026-04-25` 起，质量工场阶段 3 覆盖治理基础已新增 `scripts/auto/generate-acceptance-coverage.mjs`：脚本从 `config/automation/acceptance-registry.json` 与 `config/automation/business-acceptance-packages.json` 离线生成 `acceptance-coverage-*.json/.md`，用于检查 P0/P1/P2 覆盖、执行器分布、责任域、缺失场景引用、未消费场景和 P1/P2 元数据完整性；`--fail-on-gaps` 仅作为团队确认缺口可行动后的 CI/readiness gate，不替代真实环境执行验收。同日默认策略文件 `config/automation/acceptance-coverage-policy.json` 已作为 readiness gate 基线：缺失场景引用、P1/P2 元数据缺口和 P0/P1 最小覆盖不足为 error；baseline 未纳包和 runner 分布缺口先按 warning 进入报告。同日覆盖治理继续补齐 `scripts/auto/diff-acceptance-coverage.mjs`，可对比最近两份或显式指定的 `acceptance-coverage-*.json`，输出 `acceptance-coverage-diff-*.json/.md`，用于封板和周回归判断覆盖趋势是否改善、持平、混合变化或回退。同日新增 `scripts/auto/run-acceptance-readiness.mjs` 作为组合入口：默认聚合覆盖矩阵、策略门禁和可用时的覆盖趋势，对外输出 `acceptance-readiness-*.json/.md` 并统一回答 `passed / warning / failed`，仅证明自动化资产是否具备验收准备状态，不替代真实 `application-dev.yml` 环境业务验收。同日结果证据继续补齐 `scripts/auto/generate-automation-result-archive-index.mjs`：它从 `logs/acceptance/registry-run-*.json` 生成 `automation-result-index.latest.json` 与时间戳 `json/md` 产物，供 `/api/report/automation-results/page|recent|facets` 和自动化治理台结果证据工作区优先消费，支持按 `runId / status / runnerType / packageCode / environmentCode / 日期范围` 检索历史运行；`2026-04-26` 起 latest index 还会一并沉淀 `failureSummary / failedModules / failedScenarios` 只读失败归因，业务验收结果页对失败模块展示 `主分类 / 判断理由 / 证据摘要`，自动化治理台结果证据展示运行级失败分类分布和场景级归因，但不改变 `passed / failed / blocked`；它只提升文件证据检索性，不替代真实环境执行验收，也不引入数据库真相表。
...
`2026-04-26` 起，风险指标目录真相源已切到契约字段页显式治理：只有 `iot_product.metadata_json.objectInsight.customMetrics[]` 中满足 `group=measure && enabled=true && includeInTrend=true` 且仍命中正式 `property` 合同的字段，才允许进入 `risk_metric_catalog`；改成 `status/runtime`、`enabled=false` 或执行 `取消趋势展示` 后必须同步退役出目录。合同发布后仍优先带真实 `releaseBatchId` 自动同步目录；若历史产品已有正式字段但尚未形成发布批次，契约字段保存、`GET /api/risk-point/devices/{deviceId}/formal-metrics` 读侧自愈与待治理候选读侧自愈都会按当前正式字段现状重建目录，允许 `zhd-monitor-multi-displacement-v1` 这类无批次历史产品也把被明确设为监测数据的正式字段暴露给风险绑定。正式绑定候选仍只返回 `risk_metric_catalog` 字段，不从 `iot_product_model` 或设备全量属性旁路兜底。
