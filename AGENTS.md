...
`2026-04-18` 起，`接入智维` 资产底座新增 `/device-onboarding` 无代码接入台，作为零代码接入的第一阶段基础工作台。当前统一以 `iot_device_onboarding_case` 承载接入案例，支持分页、新建、编辑与状态刷新，并固定按 `PROTOCOL_GOVERNANCE / PRODUCT_GOVERNANCE / CONTRACT_RELEASE / ACCEPTANCE` 四步派生当前步骤：缺 `protocolFamilyCode / decryptProfileCode / protocolTemplateCode` 时判为 `BLOCKED` 并引导 `/protocol-governance`，协议信息完整但未绑定产品时判为 `PRODUCT_GOVERNANCE + BLOCKED`，已绑定产品但未形成 `releaseBatchId` 时判为 `CONTRACT_RELEASE + IN_PROGRESS` 并优先引导 `/products/:productId/contracts`（缺产品主键时回到 `/products` 主列表，旧 query 深链继续兼容），只有存在正式合同批次时才进入 `ACCEPTANCE + READY`。该页当前只交付“统一接入 intake + 卡点收口”基础版，不代表任意设备厂商已经实现零代码接入。
