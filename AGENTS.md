...
`2026-04-26` 起，真实环境 schema sync 还会对共享库已知的 `sys_dict.uk_dict_code_tenant` 唯一性漂移执行窄口自修复：只有当列顺序完全一致、差异仅体现在 `UNIQUE / INDEX` 时才允许先验重建；若检测到重复行或其他结构差异，脚本仍会直接中止并输出阻塞明细。
