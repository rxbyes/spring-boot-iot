...
`2026-04-25` 起，可观测证据链第一期新增 `sys_business_event_log` 与 `sys_observability_span_log` 两张持久化证据表：`/api/*` 会沉淀 HTTP 调用片段，登录与非 GET 业务动作会沉淀业务事件；设备上行 Pipeline 在 Redis `message-flow` 之外会保留持久化调用片段摘要；调度任务运行台账与慢 SQL 也会进入统一可检索证据链。后端读侧已开放 `/api/system/observability/business-events/page`、`/api/system/observability/spans/page`、`/api/system/observability/scheduled-tasks/page`、`/api/system/observability/message-archive-batches/page`、`/api/system/observability/message-archive-batches/report-preview`、`/api/system/observability/spans/slow-summary`、`/api/system/observability/spans/slow-trends` 与 `/api/system/observability/trace/{traceId}` 作为证据包入口和慢点/调度热点入口；调度台账会按每次 `@Scheduled` 执行沉淀 `taskCode / triggerType / triggerExpression / duration / status / traceId`，归档批次台账分页则用于核查 `iot_message_log_archive_batch` 的确认报告、归档行数、删除行数与失败原因，报告预览接口则按 `batchNo` 只读回显仓库 `logs/observability/` 下的 JSON 摘要与 Markdown 预览。前端 `/system-log` 已接入 `调度任务台账`、`性能慢点 Top`、`归档批次台账`、归档批次 `批次号 / 状态 / 创建日期` 筛选、慢点 span 明细下钻、慢点趋势下钻、归档批次详情抽屉中的 `确认报告预览` 与行级 `证据` 抽屉，支持按热点查看最近 `24` 小时 / `7` 天 bucket 趋势、`P95 / P99 / 错误率`，并从最近调度任务、最新慢点、归档批次或异常记录复盘同一证据链上的业务事件、调用片段、批次确认结果与合并时间线；若某批次仅保留 JSON 报告而未生成 Markdown，也按“部分可预览”继续展示摘要。
