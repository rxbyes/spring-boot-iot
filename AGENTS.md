...
2026-04-24 起，设备资产中心的 `查看详情` 与 `设备操作` 已拆成两个独立抽屉：详情只读承接资产主档与运行台账，设备操作单独承接能力与命令台账；能力抽屉顶部已补齐空白 / 最近一次 / 保守 / 演示四种参数模板入口，并会自动回填最近一次参数草稿，同页还支持收藏当前参数为常用命令并一键回填，并会按广播/云台类能力自动生成专用预设，无可执行能力设备需在能力抽屉顶部明确提示不可操作原因，不得再把操作职责塞回详情页。
