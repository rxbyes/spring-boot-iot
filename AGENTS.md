...
`2026-04-16` 起，协议安全运行态已切到 `published snapshot -> YAML fallback`：`iot_protocol_family_definition_snapshot / iot_protocol_decrypt_profile_snapshot` 中的 `PUBLISHED` 快照会优先重建 family 与 decrypt profile，`family:<familyCode>` 校验和 legacy `$dp` decrypt resolver 也统一复用该口径；`iot_vendor_metric_mapping_rule_snapshot` 同时成为正式运行态真相，只要产品存在任意 `PUBLISHED` 映射快照，运行时与命中预览就只按快照判定，只有产品尚无正式快照时才回退草稿表。
