...
`2026-04-25` 起，可观测证据链第一期新增 `sys_business_event_log` 与 `sys_observability_span_log` 两张持久化证据表：`/api/*` 会沉淀 HTTP 调用片段，登录与非 GET 业务动作会沉淀业务事件；设备上行 Pipeline 在 Redis `message-flow` 之外会保留持久化调用片段摘要；调度任务异常与慢 SQL 也会进入统一可检索证据链。后端读侧已开放 `/api/system/observability/business-events/page`、`/api/system/observability/spans/page`、`/api/system/observability/spans/slow-summary` 与 `/api/system/observability/trace/{traceId}` 作为证据包入口和慢点热点入口，前端 `/system-log` 已接入 `性能慢点 Top` 面板、慢点 span 明细下钻与行级 `证据` 抽屉，支持从最新慢点、明细 span 或异常记录复盘同一 TraceId 下的业务事件、调用片段与合并时间线。
