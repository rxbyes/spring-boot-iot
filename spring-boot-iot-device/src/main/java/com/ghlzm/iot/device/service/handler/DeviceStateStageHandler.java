...
public class DeviceStateStageHandler {
    ...
    public void refresh(DeviceProcessingTarget target) {
        ...
        deviceSessionService.online(target.getDevice().getDeviceCode(), clientId);
        deviceSessionService.refreshLastSeen(target.getDevice().getDeviceCode(), clientId, target.getMessage().getTopic());
    }
