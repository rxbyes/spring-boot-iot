...
- `接入智维` 页面当前统一减少重复导航层级：只保留全局壳层面包屑这一处“接入智维 / 当前模块”定位。单主列表页收口为“全局面包屑 + 主列表标题 + 共享筛选/工具条/结果区”两层语法；`/products` 主卡标题保留 `产品定义中心`，`/devices` / `/system-log` 分别保留 `设备台账` / `异常台账`。`/reporting`、`/message-trace` 与 `/file-debug` 这类真实页签诊断页则保留“全局面包屑 + 真实页签 + 业务标题”，不再额外渲染页内同路径面包屑、页内标题壳或跨页功能菜单。
