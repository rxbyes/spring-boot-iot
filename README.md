...
3. 若是历史库升级（非新库初始化），按 [docs/04-数据库设计与初始化数据.md](docs/04-数据库设计与初始化数据.md) 执行 `sql/upgrade/` 增量脚本；菜单结构升级到五工作台时，需执行 `sql/upgrade/20260319_phase5_workspace_menu_refactor.sql`，设备资产中心按钮权限缺失时再执行 `sql/upgrade/20260319_phase5_device_asset_center.sql`
...
- `/devices` 已升级为设备资产中心，支持设备分页筛选、详情查看、新增、编辑、单删、批量删除和 CSV 导出。
