...
- `/message-trace` 详情抽屉当前已切到 `A 完整内容平衡版` 刊栏工作台：抽屉标题行承担轻刊头，正文完整保留 `消息概览 lead-sheet`、`链路信息（章内拆成 链路标识 / 接入上下文）`、`Payload 对照`、`处理时间线` 四个章节，首屏视觉以附件终版的大标题 lead-sheet 和右侧指标列为准，不再回退到设备详情 hero 语法。`Payload 对照` 继续固定展示 `原始 Payload / 解密后明文 / 解析结果` 三栏，但原始 Payload 现在来自消息日志详情恢复接口，解密明文与解析结果优先按当前协议从已落库消息日志重算，不再只依赖 `message-flow` 的 `PROTOCOL_DECODE.summary`；即使 Redis 时间线已过期或查询异常，仍可继续查看恢复后的明文与解析结果；若详情恢复接口返回空字符串或空对象，前端也会继续基于原始 Payload 做兜底展示，避免出现“原始有值但明文/解析为空”的误导状态。
