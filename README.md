...
- 2026-03-19 起，`/products` 已升级为产品定义中心首个可用闭环，支持产品分页筛选、详情查看、新增、编辑、删除、CSV 导出，以及按产品查看关联库存设备；2026-03-21 起，产品详情已补齐设备活跃度统计（今日/7日/30日活跃）和在线时长分析（平均/最长在线时长）。在线时长基于 `iot_device_online_session` 会话明细统计，历史老数据不回填。
- 若真实环境历史库暂未执行 `sql/upgrade/20260321_phase5_device_online_session.sql`，服务端当前会自动降级跳过在线会话明细写入与聚合，不再因为 `iot_device_online_session` 缺表阻断 `$dp` / MQTT / HTTP 上报主链路；此时产品详情在线时长统计返回 `null` 属于预期。
- 2026-03-20 起，`/devices` 已升级为设备资产中心第三轮可用闭环，支持设备分页筛选、详情查看、新增、编辑、批量导入、设备更换、单删、批量删除、CSV 导出，以及父设备/子设备拓扑关系维护与展示。
- 2026-03-21 起，前端右上角 `通知中心 / 帮助中心` 已升级为角色化壳层能力：通知按当前角色、工作台、最近操作和失败记录聚合系统/业务/错误事件；帮助按当前菜单权限筛出业务类、技术类、FAQ 资料，不再展示无权限入口。2026-03-21 晚间起，前端壳层已优先接入 `/api/system/in-app-message/my/*` 与 `/api/system/help-doc/access/*`，本地壳层内容仅作兜底；同日继续补齐“查看更多”抽屉、详情抽屉、帮助全文检索高亮与服务端分页，通知未读改为显式已读，不再因打开摘要面板自动清空。
- 2026-03-21 起，`spring-boot-iot-system` 已补齐右上角壳层的后端最小闭环：新增站内消息、已读记录、帮助文档三张系统治理表，以及 `站内消息管理 / 我的消息 / 帮助文档管理 / 权限过滤访问` 接口，后续前端可直接替换本地壳层数据源。
- 2026-03-21 夜间起，平台治理前端已补齐 `/in-app-message` 与 `/help-doc` 两个系统内容编排页，管理/超管可直接维护通知中心消费的站内消息和帮助中心消费的帮助文档，不再只停留在壳层消费端。
- 2026-03-22 起，帮助中心与 `docs/` 文档体系已开始按“权威资料层 + 消费层”联动治理：`docs/` 继续作为研发/运维权威资料源，帮助中心继续通过 `/api/system/help-doc/**` 维护面向角色和页面的精简消费版；`init-data.sql` 与 `20260321_phase5_in_app_message_help_docs.sql` 当前已补齐首批 13 篇帮助文档样例。
- 2026-03-22 起，通知中心已继续从“可消费壳层”升级为“可自动生产 + 可治理追踪”的站内消息中心：后端新增统一内部发布契约 `InAppMessagePublisher` / `InAppMessagePublishCommand`，并已接入 `system_error`、事件派工、工单接收/开始/完成三类自动消息源；管理端同步补齐来源类型筛选、自动消息停用治理和通知统计概览。
- 2026-03-22 深夜起，通知中心已补齐“高优未读阈值后桥接通知渠道”闭环：当 `high / critical` 站内消息超过阈值仍未读时，后端会按调度扫描复用既有通知渠道模块进行桥接；桥接继续以 `scenes` 驱动，不把通知中心扩成新的多渠道编排产品。
- Phase 4 当前已完成并进入真实环境验收基线的能力：告警运营台、事件协同台、风险对象中心、阈值策略、联动编排、应急预案库、运营分析中心，以及平台治理下的组织架构、账号中心、角色权限、导航编排、区域版图、数据字典、通知编排、站内消息、帮助文档、审计中心；接入智维分区已补齐“异常观测台”和“链路追踪台”，用于按 `TraceId`、设备编码、产品标识与 Topic 查看 `system_error` 后台异常和接入链路问题。
- 2026-03-21 起，深部位移 `$dp` 历史兼容链路已补齐平台内自动闭环：子设备 `dispsX / dispsY` 入库后会按 `risk_point_device` 绑定自动完成红橙黄蓝判级、黄橙红告警、橙/红事件与自动派工，并把联动规则/应急预案命中结果写入告警备注和事件复盘留痕；应急预案当前仅命中深部位移场景化预案，已升级实例之间会额外使用 Redis 分布式锁压缩同测点并发重复建单。
- 2026-03-22 起，`/reporting`（链路验证中心）已升级为设备编码反查驱动的联调工作台：支持 HTTP / MQTT 双通道、明文 / 密文双模式、JSON / XML 文本格式化、`$dp` 默认 Topic 自动补 `deviceCode`（MQTT 场景）以及“模拟上报 + 诊断复盘”双主卡等高布局。
- 2026-03-19 起，角色默认首页已按职责分化：业务/管理优先进入 `风险运营`，运维/研发优先进入 `接入智维`，超级管理员优先进入 `平台治理`。
- 2026-03-20 起，前端工作台导航、分组首页、路由 `title/description` 与角色默认落点已统一收口到共享 workspace schema；顶部搜索同步升级为 `Ctrl/Cmd + K` 全局命令面板，原最近访问标签栏与 `trackTab` 遗留机制已移除。
...
- 产品不是单台设备，而是“一组接入身份、协议契约、数据模型和生命周期规则一致的设备模板”；设备才是现场安装的具体资产实例。
- 当前代码已要求“先产品、后设备”：新增设备必须提供 `productKey`，服务端会先解析产品，再把该产品的 `protocolCode`、`nodeType` 等基础属性继承到设备。
- 设备建档当前已支持补充父子拓扑：新增/编辑/设备更换可直接传 `parentDeviceId`，批量导入可传 `parentDeviceCode`，详情与列表会同步展示父设备和关联网关信息。
- `productKey` 是机器侧接入身份，既用于 `POST /api/device/add` 建档，也用于 MQTT Topic、消息日志和审计日志中的产品标识；当前代码已禁止创建后修改 `productKey`。
- `productName` 是人读名称，建议按“厂商 + 场景/用途 + 品类 [+ 接入方式/系列/版本]”命名，例如 `南方测试 监测型 GNSS`、`中海达 预警型 广播终端`。
- `manufacturer` 只记录厂商品牌/制造商，例如 `南方测试`、`中海达`；不要把设备品类、项目名、区域名混入厂商字段。
- 建议按“接入契约”拆分产品：只要协议编码、物模型、上下行指令、认证方式、节点类型或生命周期治理规则不同，就应视为不同产品；项目、站点、安装位置等实例信息应落到设备侧。
- 当前代码已在删除产品前校验关联库存设备，并在停用产品前校验是否仍存在启用中的关联设备；停用产品不会再出现在设备建档下拉中，且会被后端拒绝继续建档、设备上报接入和 MQTT 下行指令下发。
...
- `docs/` 目录与源码、SQL 一起构成项目事实基础；帮助中心是从这些权威资料中提炼出来的消费层，不是第二套平行真相源。
- 当前帮助中心仍只使用既有 `business / technical / faq` 三分类，以及 `visible_role_codes + related_paths + currentPath` 的角色/页面过滤与排序模型。
- `sql/init-data.sql` 与 `sql/upgrade/20260321_phase5_in_app_message_help_docs.sql` 已补齐首批 13 篇样例：`5` 篇业务类、`4` 篇技术类、`4` 篇 FAQ，覆盖产品设备建档、告警/事件闭环、风险策略、报表分析、HTTP/MQTT 接入、TraceId 排障、真实环境启动与高频权限问题。
- 后续凡是功能、接口、流程、配置发生变化，先更新 `docs/` 权威文档，再评估是否同步更新 `/help-doc` 的摘要版内容。
...
- 若未显式覆盖 `IOT_MQTT_CLIENT_ID`，默认值 `spring-boot-iot-mqtt-consumer` 会在运行时自动派生 `主机/PID` 后缀，降低共享 Broker 上多实例互踢连接的风险；如需固定值，请设置自定义且非默认的 `clientId`。

历史 `$dp` 基准站一包多测点联调补充：
- 当前已支持通过 `iot.device.sub-device-mappings` 把基准站逻辑测点映射到真实子设备 `device_code`，用于南方测绘深部位移等历史兼容场景。
- 若共享库中还没有 `SK00FB0D1310195` 的 8 个子设备和初始风险点，可执行 `sql/upgrade/20260320_phase4_deep_displacement_sub_devices_bootstrap.sql`。
- 深部位移自动闭环命中应急预案时，当前只会选择名称、描述、响应步骤、联系人等文本中能命中“深部位移/位移/边坡/滑坡”等场景关键词或风险点上下文的启用预案；若库里暂无场景化预案，留痕会保留 `emergencyPlan=null`，不再误挂到通用锅炉类预案。
- 已升级实例之间会额外使用 Redis 锁 `iot:alarm:auto-closure:{deviceCode}:{metricIdentifier}:{alarmLevel}` 抑制并发重复建单；若共享 Broker/数据库环境中仍有旧版 consumer 未升级，同一条 MQTT 上报仍可能出现新旧两套留痕并存，需要统一升级后再判断最终验收结果。

## 系统异常审计与通知
- MQTT 启动失败、订阅失败、连接断开、消息分发失败等后台异步异常会自动写入 `sys_audit_log`，其中 `operation_type=system_error`、`request_method=MQTT`、`user_name=SYSTEM`。
- `/api/**` 入口会统一生成或透传 `X-Trace-Id`，并同步写入 `sys_audit_log.trace_id` 与 `iot_device_message_log.trace_id`，用于跨 HTTP、MQTT、系统日志和消息日志串联排查。
...
- 2026-03-22 起，后台异常审计链路除现有 webhook/企业消息通知外，还会统一发布站内 `error` 消息到通知中心，目标角色固定为 `OPS_STAFF / DEVELOPER_STAFF / SUPER_ADMIN`，默认 24 小时过期。
- 2026-03-22 起，事件派工与工单状态流转也会统一走站内消息内部发布器：
  - 事件派工：发给 `receiveUser`，来源类型 `event_dispatch`，默认 7 天过期。
  - 工单接收 / 开始处理：发给处理人 `receiveUser`，来源类型 `work_order`，优先级 `medium`。
  - 工单完成：发给派工人 `assignUser`，来源类型 `work_order`，优先级 `high`。
- 2026-03-22 深夜起，可选开启“高优未读桥接通知渠道”：
  - 仅扫描 `high / critical` 且超过阈值仍未读的站内消息。
  - 默认桥接场景名为 `in_app_unread_bridge`，仍复用 `webhook / wechat / feishu / dingtalk` 现有渠道模块。
  - 当前采用“每条站内消息 + 每个渠道一次成功闭环、失败按渠道冷却重试”的策略，并把结果写入 `sys_in_app_message_bridge_log`。
...
- `sys_audit_log.result_message` 当前按真实库 `VARCHAR(500)` 口径入库；超长系统异常消息会在服务端自动截断并追加 `...(truncated)`，避免审计记录自身再触发 `Data too long` 写入失败。
- 若历史库尚未执行 `sql/upgrade/20260321_phase5_in_app_message_help_docs.sql`，`/api/system/help-doc/**` 与 `/api/system/in-app-message/**` 相关接口现在会明确返回升级脚本提示；若是 2026-03-21 之前已落过旧版系统内容表结构，还应继续执行 `sql/upgrade/20260322_phase5_notification_center_followup.sql` 补齐 `dedup_key`、索引和来源口径回填。
- 如需开启系统异常自动通知，设置 `IOT_OBSERVABILITY_SYSTEM_ERROR_NOTIFY_ENABLED=true`。
- 如需开启高优未读桥接，设置 `IOT_OBSERVABILITY_IN_APP_UNREAD_BRIDGE_ENABLED=true`，并按需覆盖 `IOT_OBSERVABILITY_IN_APP_UNREAD_BRIDGE_SCAN_INTERVAL_SECONDS`、`IOT_OBSERVABILITY_IN_APP_UNREAD_BRIDGE_HIGH_THRESHOLD_MINUTES`、`IOT_OBSERVABILITY_IN_APP_UNREAD_BRIDGE_CRITICAL_THRESHOLD_MINUTES`。
- 当前自动通知支持 `webhook`、`wechat`、`feishu`、`dingtalk` 渠道类型；渠道 `config` JSON 至少需要 `url`，系统异常自动通知要求 `scenes` 中包含 `system_error`，高优未读桥接要求 `scenes` 中包含 `in_app_unread_bridge`。
...
3. 若是历史库升级（非新库初始化），按 [docs/04-数据库设计与初始化数据.md](docs/04-数据库设计与初始化数据.md) 执行 `sql/upgrade/` 增量脚本；菜单结构升级到五工作台时，需执行 `sql/upgrade/20260319_phase5_workspace_menu_refactor.sql`，设备资产中心按钮权限缺失或仍未补齐批量导入/设备更换权限时再执行 `sql/upgrade/20260319_phase5_device_asset_center.sql`；若历史库尚未补齐站内消息与帮助文档能力，再执行 `sql/upgrade/20260321_phase5_in_app_message_help_docs.sql`；若历史库已具备系统内容表但仍缺通知来源治理、`dedup_key` 或索引，再执行 `sql/upgrade/20260322_phase5_notification_center_followup.sql`；若本轮需要开启高优未读桥接通知渠道，再继续执行 `sql/upgrade/20260322_phase5_notification_channel_bridge.sql`；若历史库还未补齐平台治理下的 `站内消息 / 帮助文档` 菜单和按钮授权，再执行 `sql/upgrade/20260321_phase5_system_content_menu_governance.sql`
   若直接访问 `/api/system/help-doc/**` 或 `/api/system/in-app-message/**` 出现“系统内容依赖表缺失，请先执行 `sql/upgrade/20260321_phase5_in_app_message_help_docs.sql`，若为旧库升级请继续执行 `sql/upgrade/20260322_phase5_notification_center_followup.sql`”，说明当前真实库仍缺少 2026-03-21 至 2026-03-22 的系统内容 schema，对应脚本执行完成后无需切回旧验收路径。
   若共享开发库中的演示账号、角色名称或新补齐按钮菜单出现 `????` / `?????`，执行 `sql/upgrade/20260319_phase5_demo_text_repair.sql` 修复中文文本
4. 启动前先在仓库根目录执行一次全量安装，确保多模块依赖中的最新 Java 类都已编译并安装到本地 Maven 仓库，避免直接运行 `spring-boot-iot-admin` 时遗漏其他模块编译结果：
   ```bash
...
6. 启动前端：
   ...
   ```
7. 按 [docs/test-scenarios.md](docs/test-scenarios.md) 和 [docs/21-business-functions-and-acceptance.md](docs/21-business-functions-and-acceptance.md) 进行真实环境验收
...
- `/products` 已升级为产品定义中心，支持产品分页筛选、详情查看、新增、编辑、删除、CSV 导出和按产品查看库存设备；详情接口会额外返回今日/7日/30日活跃统计字段。
- `/devices` 已升级为设备资产中心，支持设备分页筛选、详情查看、新增、编辑、批量导入、设备更换、单删、批量删除、CSV 导出，以及父设备/子设备拓扑关系维护与展示。
- `/reporting` 已升级为链路验证中心：设备编码是唯一查询入口，查询成功后只读反显 `productKey / protocolCode / clientId`，支持 HTTP / MQTT 明文或密文模拟上报，`tenantId` 默认 `1` 且仅 HTTP 生效，`topic` 默认 `$dp` 且允许手改，JSON / XML / 普通文本可按规则格式化与预演。
- 当前质量工场新增 `/automation-test` 自动化工场，用于前端配置场景模板并导出配置驱动浏览器巡检计划；当前已支持截图基线路径配置、页面/局部截图断言与视觉回归参数配置
- 当前公共壳层已统一为阿里云式浅色控制台布局：顶部采用“品牌 / 搜索 / 工具区 + 一级业务分区条”，左侧采用简化二级菜单，内容区功能页顶部仅保留阿里云式面包屑，不再显示最近访问标签栏，也不再重复渲染主标题、同组横向导航以及“当前分区 / 页面类型 / 可见菜单 / 返回概览”等说明性信息
...
- 若历史库里 `SUPER_ADMIN` 的真实主键不是 `92000005`，请使用当前仓库版本的 `20260316_phase4_task10_dynamic_menu_auth.sql`；若需把旧三大分区菜单升级为五工作台，请低峰执行 `sql/upgrade/20260319_phase5_workspace_menu_refactor.sql`；若需补齐产品定义中心按钮权限，请执行 `sql/upgrade/20260319_phase5_product_definition_center.sql`；若仅需补齐导航编排按钮权限，可低峰执行 `sql/upgrade/20260317_phase4_menu_button_permission_backfill.sql`
...
- 业务页面默认要求登录；未登录、token 失效或访问未授权/无资源路由时，前端会清理本地登录态并跳转到 `/login`
- 除公共白名单外，后端接口默认要求携带 `Authorization: Bearer <token>`
- 当前公共接口：`/api/auth/login`、`/api/message/http/report`、`/api/cockpit/**`、`/actuator/**`、`/doc.html`、`/swagger-ui/**`、`/v3/api-docs/**`
- 前端已接入 token 持久化、`authContext` 恢复与请求头自动注入；刷新进入业务页时会优先恢复本地登录上下文，后端返回业务 `401` 或 HTTP `401` 时再统一回到 `/login`
- 同源部署下，浏览器直接刷新 `/`、`/products`、`/risk-disposal` 以及更深层的前端 history 路由时，后端会先回退到 SPA `index.html`，再由前端决定保留当前页或跳转登录页，不再直接显示原始 `401` JSON
- `dev` 基线下 token 默认有效期为 24 小时，可通过环境变量 `IOT_SECURITY_TOKEN_EXPIRE_SECONDS` 覆盖
