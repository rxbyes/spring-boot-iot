...
3. 若是历史库升级（非新库初始化），按 [docs/04-数据库设计与初始化数据.md](docs/04-数据库设计与初始化数据.md) 执行 `sql/upgrade/` 增量脚本；菜单结构升级到五工作台时，需执行 `sql/upgrade/20260319_phase5_workspace_menu_refactor.sql`，设备资产中心按钮权限缺失或仍未补齐批量导入/设备更换权限时再执行 `sql/upgrade/20260319_phase5_device_asset_center.sql`
