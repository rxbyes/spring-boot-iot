...
3. 首次升级到当前验收基线时，按 [docs/03-database.md](docs/03-database.md) 依次执行 `sql/upgrade/` 下的升级脚本，至少包含 `20260316_phase4_task3_risk_monitoring_schema_sync.sql` 与 `20260316_iot_message_log_view.sql`
4. 启动后端：
   ```bash
...
- 共享开发库若处于早期 Phase 4 半升级状态，风险监测联调前必须执行 `sql/upgrade/20260316_phase4_task3_risk_monitoring_schema_sync.sql`，否则 `/api/risk-monitoring/*` 会因缺少 `risk_point_device` 或旧版 `risk_point` 字段而阻塞
...
- 前端已接入 token 持久化与请求头自动注入；当接口返回 `401` 时会清理本地登录态
