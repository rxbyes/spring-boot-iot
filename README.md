...
- 2026-03-21 起，前端右上角 `通知中心 / 帮助中心` 已升级为角色化壳层能力：通知按当前角色、工作台、最近操作和失败记录聚合系统/业务/错误事件；帮助按当前菜单权限筛出业务类、技术类、FAQ 资料，不再展示无权限入口。2026-03-21 晚间起，前端壳层已优先接入 `/api/system/in-app-message/my/*` 与 `/api/system/help-doc/access/*`，本地壳层内容仅作兜底。
