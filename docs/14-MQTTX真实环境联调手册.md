...
- 若健康检查正常、代理路径也一致，但脚本仍只拿到 `PUBACK` 且查不到对应 payload 的 `iot_message_log` / `iot_device_access_error_log`，当前共享 Broker 可能不会把平台侧手工 publish 的 `$dp` 报文回投给 consumer；此时要切换到真实设备、专用模拟器或可控 Broker 环境继续验收。
