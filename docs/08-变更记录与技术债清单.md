...
5. 设备资产中心当前仅交付首个可用闭环，批量导入、设备更换、远程维护与维修工单联动仍待后续补齐。
...
- 2026-03-19：修复 Vite 开发代理把前端 `/devices` history 路由误匹配到后端代理前缀的问题；当前开发代理已统一收紧到 `^/api(?:/|$)`、`^/message(?:/|$)`、`^/device(?:/|$)`，避免 `/devices`、`/device-access`、`/message-trace` 这类前端路由在强制刷新时被误转发到后端并因缺少 `index.html` 静态资源返回 `500`。
