...
- `/devices` 已开始承接 `/products` 验证通过的高频台账体验方案，但本轮仍保持“Hero 卡 + 独立筛选卡 + 台账结果卡”三段结构，未直接强行并到单卡。保留原因是设备页当前有 7 个高频筛选项，继续全部压到单卡首屏会明显挤压台账可读性。
...
- 设备页筛选区现已收口为“默认高频条件 + 更多条件”结构：首屏默认保留设备编码、设备名称、产品 Key、在线状态 4 项，设备 ID、激活状态、设备状态 收到“更多条件”中；当隐藏条件已填写但面板收起时，页面会显式提示“更多条件中已填写 N 项”，避免条件静默生效。
- 本轮新增回归：
...
- 当前仍保留的优化空间：
  - 设备页当前仍主要面向桌面表格；后续若要继续优化移动端观感，应补齐与桌面同源的卡片列表模式，而不是继续依赖表格横向压缩。
  - 设备详情抽屉虽然已完成上一轮层次化改造，但仍未接入“摘要秒开 + 完整详情补数 + 本地详情缓存”闭环，后续可继续沿产品页方案推广。
  - 若设备页后续还能继续减少高频条件数量，再评估是否适合把筛选条并入主卡；在当前条件规模下，继续保持独立筛选卡比强行并卡更合理。
