...
- `/message-trace` 详情抽屉当前固定采用“轻摘要 + 处理时间线 + Payload 对照 + 排查建议”结构；`Payload 对照` 必须同时保留 `原始 Payload / 解密后明文 / 解析结果` 三张等权卡片。原始 Payload 继续来自消息日志，解密明文与解析结果只从 `PROTOCOL_DECODE.summary` 读取；缺数时必须保留卡位并给出明确占位，时间线过期时只允许保留原始 Payload，解密/解析卡片统一显示“无法恢复”文案，不得再回退成单块 payload 预览或直接隐藏缺失卡片。
