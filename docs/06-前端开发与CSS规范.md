...
- 产品类详情当前至少拆分为“统一经营头部 / 核心规模主舞台 / 契约基线 / 档案摘要”四个信息层；`在线覆盖 / 30 日活跃 / 平均在线` 必须内联到核心规模主舞台，不得再拆成独立指标列，也不得把契约与档案字段重新混写成同权重摘要卡墙。
- 产品类详情当前继续收口为统一 `产品经营工作台` 抽屉，但内页视觉基线已切换为“2026 展陈型高级扁平”：`展签头部 + 目录式真页签 + 主展板 + 轻台账 / 修订板`，不再把详情页做回多层渐变卡、说明墙或政企简报式中轴对称排布。
...
- `产品经营工作台` 头部当前固定为“展签眉标 + 产品名 + 短状态句 + 去重元信息行 + 目录式真页签”五段式；不得回流左右分栏头部、中轴身份框、头部设备指标带、第二组概况卡或重复产品身份。
- `经营总览` 当前固定为“1 个核心规模主展板（内联 `在线覆盖 / 30 日活跃 / 平均在线`）+ 契约基线线表 + 档案摘要卡簇”；不得回流独立次级指标区、`当前判断` 判断块、旧的 `趋势摘要 + 接入契约与产品档案` 双表格、正文右侧指标栏或头部重复设备规模。
- `经营总览` 当前必须把 `契约基线 + 档案摘要` 收口到同一张展陈正文内：契约继续使用线表核对，档案只允许用轻附注卡承接事实字段，不得再拆成两张平权主卡或补一整段解释文。
- `物模型治理` 当前固定采用“展签头部 + 连续策展条 + 治理清单 / 正式模型工作区”结构；手动提炼区内部说明必须收口为连续治理清单，不得再回流三张并列说明卡、产品元信息 chips、`当前产品` 重复文案、侧边治理轨道或第二套正式模型页面骨架。
- `关联设备` 当前固定采用“台账横标尺 + 设备清册主区”结构；不得再保留独立章节头、重复的 `关联设备台账` 文案、台账导语或首屏高占比说明卡。
- `编辑治理` 当前固定采用“修订页眉 + 修订条 + `StandardInlineState` + 表单主区”结构；表单前不得再补导语说明，不得再回流大标题 hero、双层说明卡或让表单继续下沉到首屏之后。
