...
- `实时监测台`、`GIS态势图`、`告警运营台`、`事件协同台` 当前也统一按 `产品定义中心` 的操作列口径执行：桌面表格只允许复用 `StandardWorkbenchRowActions` 的 `table` 变体，页面不得再显式传 `gap="compact"`、`gap="wide"`、`distribution`，也不要在 `resolveWorkbenchActionColumnWidth` 上继续传私有 `gap` 参数表达“本页更紧/更宽”。
...
- `/insight` 的 `对象洞察台` 当前统一采用“单一 `StandardWorkbenchPanel` + `StandardListFilterHeader` + `StandardInlineState` + 正文内容分区”语法；相关页面联动动作统一下沉到正文卡片或行内操作，不再保留英文 hero 眉标、首屏大按钮组或第二张概况 Hero 卡。页面语义必须以 `/api/risk-monitoring/realtime/{bindingId}` 为主口径，风险等级、在线状态、监测对象档案、测点和趋势图都不得再由前端基于设备属性或消息日志自行推导；当设备未命中绑定时，只允许退化为设备上报分析视图，不允许把整页内容清空。
...
- `/devices` 的设备详情当前固定升级为与编辑页同语法的分区工作台：已登记设备采用“`基础档案` + `父子拓扑` + `状态与维护属性` + `认证字段` + `扩展信息`”结构，未登记设备切换为“`来源档案` + `失败摘要` + `最近载荷`”。设备详情标题行中的 `已登记 / 在线 / 已激活 / 启用` 状态标签当前必须与标题保持同一行展示，不得自动换行到下一层；详情、编辑与更换抽屉也不再展示“已先展示列表摘要 / 正在补全档案”一类自动补数提示带。详情字段值的字号、字重与编辑页保持同一阅读节奏，不再额外放大加粗；上述已登记板块若字段或属性全部为空，则整块隐藏，不再保留空内容占位。后续不得再把详情页回退到旧的双栏台账矩阵、小卡片墙或第二套失败详情壳。
...
- `物模型治理` 当前固定采用“刊头 + 摘要标尺 + 双证据治理工作区 / 正式模型工作区”结构；治理视图必须显式呈现 `证据入口 + 治理摘要 + 对比工作区 + 正式应用` 四段语法，不得再回流三张并列说明卡、产品元信息 chips、`当前产品` 重复文案、侧边治理轨道或第二套正式模型页面骨架。
