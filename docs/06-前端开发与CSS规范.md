...
- 纳管列表页中的 `操作` 列必须显式声明 `class-name="standard-row-actions-column"`，并通过 `resolveWorkbenchActionColumnWidth`（兼容场景可继续用 `resolveAdaptiveActionColumnWidth`）按“直出动作数量 + 是否存在更多菜单 + 标签长度”计算列宽；桌面端当前固定采用共享宽度分层：`1` 个可见动作 `96px`、`2` 个可见动作 `112px`、`前 2 项直出 + 更多` 为 `160px`。双动作桌面列不再保留过宽的旧 `144px` 下限，避免 `详情 / 观测` 这类短文案在右侧留下过大空白；出现 `更多` 时仍自动提升到三段式基线。共享全局样式继续统一处理 fixed 列裁切、字体和末尾留白；不要再依赖默认 `.cell` ellipsis，让最后一个动作后面冒出圆点，也不要再写死 `200 / 220 / 288` 一类宽度。
...
- 列表页“操作”列当前统一以 `设备资产中心` 桌面端为视觉基线：表格行内动作必须复用 `StandardWorkbenchRowActions`；桌面 `table` 变体统一使用固定共享间距，并继续搭配共享宽度分层。无论是 `2` 个动作还是 `详情 / 编辑 / 更多` 这类 `3` 个可见动作，都不得再为单页显式写 `distribution`、`gap="wide"`、页面私有列宽下限或 margin / gap hack。
...
- 产品类详情当前继续收口为统一 `产品经营工作台` 抽屉，但内页视觉基线已切换为 A 方案“扁平经营简报式”：`轻头部 + 真页签 + 主舞台 + 二级板块`，不再把详情页做回多层渐变卡和说明墙。
...
- `产品经营工作台` 头部当前固定为“唯一产品名 + 一句状态判断 + 去重元信息行 + pill 真页签”四段式；不得回流内部 kicker、头部设备指标带、第二组概况卡或重复产品身份。
- `经营总览` 当前固定为“1 个核心规模主舞台（内联 `在线覆盖 / 30 日活跃 / 平均在线`）+ 契约基线线表 + 档案摘要卡”；不得回流独立次级指标区、`当前判断` 判断块、旧的 `趋势摘要 + 接入契约与产品档案` 双表格、正文右侧指标栏或头部重复设备规模。
- `物模型治理` 当前固定采用“轻头部 + 3 条摘要 + 扁平工作区”结构；候选提炼与正式模型必须共享同一套语法，不得再回流产品元信息 chips、`当前产品` 重复文案、侧边治理轨道或第二套正式模型页面骨架。
- `关联设备` 当前固定采用“紧凑指标带 + 单条台账导语 + 台账主区”结构；不得再保留独立章节头、重复的 `关联设备台账` 文案或首屏高占比说明卡。
- `编辑治理` 当前固定采用“章节短标题 + 3 卡上下文条 + `StandardInlineState` + 表单主区”结构；不得再回流大标题 hero、双层说明卡或让表单继续下沉到首屏之后。
