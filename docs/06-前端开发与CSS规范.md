...
- `/message-trace` 详情抽屉当前固定采用 `A 完整内容平衡版` 刊栏语法：`StandardDetailDrawer` 标题行承担轻刊头，正文完整保留 `消息概览 lead-sheet + 链路信息 + Payload 对照 + 处理时间线` 四个章节，首屏视觉必须以附件终版的大标题 lead-sheet 和右侧指标列为准，不得回退到设备详情 hero 语法。链路信息不得再回退成一整块无分组台账，至少要拆成 `链路标识` 与 `接入上下文` 两组阅读单元。`Payload 对照` 必须同时保留 `原始 Payload / 解密后明文 / 解析结果` 三张等权卡片，其中原始 Payload 来自消息日志，解密明文与解析结果优先来自详情恢复接口重建结果，`PROTOCOL_DECODE.summary` 仅作为补充回退；若详情恢复接口返回空字符串、空对象或等价空值，前端必须继续基于原始 Payload 做兜底，不得把“有原始报文”的链路渲染成明文/解析双空。处理时间线必须继续作为独立全文宽章节承接固定 Pipeline 复盘；时间线过期时不得把 payload 对照回退成只剩原始 Payload，也不得继续显示“无法恢复”误导文案。
