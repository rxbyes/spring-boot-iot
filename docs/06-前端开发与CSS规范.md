...
- 纳管列表页中的 `操作` 列必须显式声明 `class-name="standard-row-actions-column"`，并通过 `resolveWorkbenchActionColumnWidth`（兼容场景可继续用 `resolveAdaptiveActionColumnWidth`）按“直出动作数量 + 是否存在更多菜单 + 标签长度”计算列宽；桌面端当前固定采用共享宽度分层：`1` 个可见动作 `96px`、`2` 个可见动作 `112px`、`前 2 项直出 + 更多` 为 `160px`。双动作桌面列不再保留过宽的旧 `144px` 下限，避免 `详情 / 观测` 这类短文案在右侧留下过大空白；出现 `更多` 时仍自动提升到三段式基线。共享全局样式继续统一处理 fixed 列裁切、字体和末尾留白，并把 `standard-row-actions-column .cell` 固定为右贴齐，让短文案页与长文案页共用同一条右侧基线；不要再依赖默认 `.cell` ellipsis，让最后一个动作后面冒出圆点，也不要再写死 `200 / 220 / 288` 一类宽度。
...
- 风险运营模块下的 `实时监测台`、`GIS态势图`、`告警运营台`、`事件协同台` 当前与上述基线完全一致：单动作页走 `96px` 档位，多动作页继续按 `112 / 160px` 档位收口；`GIS` 的卡片操作区继续使用共享 `card` 变体默认节奏，不再另起风险运营私有移动端间距规则。
...
- `/devices` 的设备详情当前固定升级为扁平化工作台：已登记设备采用“`资产状态与接入概况` + `身份与部署台账` + `运行与认证台账` + `关系与建档补充 / 扩展元数据快照`”结构；未登记设备继续复用同一 `StandardDetailDrawer`，但主账本切换为“`来源档案与失败摘要` + `最近载荷`”，不再并排维护第二套失败详情壳。
- 产品类详情当前至少拆分为“统一经营头部 / 核心规模主舞台 / 契约基线 / 档案摘要”四个信息层；`在线覆盖 / 30 日活跃 / 平均在线` 必须内联到核心规模主舞台，不得再拆成独立指标列，也不得把契约与档案字段重新混写成同权重摘要卡墙。
- 产品类详情当前继续收口为统一 `产品经营工作台` 抽屉，但内页视觉基线已切换为 A 方案下的“庄重对称政企简报式”：`中轴简报头部 + 真页签 + 主舞台 + 正式附注层`，不再把详情页做回多层渐变卡和说明墙。
...
- `产品经营工作台` 头部当前固定为“中轴标题 + 一句状态判断 + 去重元信息行 + 居中 pill 真页签”四段式；不得回流左右分栏头部、内部 kicker、头部设备指标带、第二组概况卡或重复产品身份。
- `经营总览` 当前固定为“1 个核心规模主舞台（内联 `在线覆盖 / 30 日活跃 / 平均在线`）+ 纵向契约基线线表 + 纵向档案摘要附注层”；不得回流独立次级指标区、`当前判断` 判断块、旧的 `趋势摘要 + 接入契约与产品档案` 双表格、正文右侧指标栏或头部重复设备规模。
- `物模型治理` 当前固定采用“中轴标题 + 3 条正式摘要带 + 顶部流程条 / 正式模型工作区”结构；候选提炼与正式模型必须共享同一套语法，不得再回流产品元信息 chips、`当前产品` 重复文案、侧边治理轨道或第二套正式模型页面骨架。
- `关联设备` 当前固定采用“正式摘要带 + 台账主区”结构；不得再保留独立章节头、重复的 `关联设备台账` 文案或首屏高占比说明卡。
- `编辑治理` 当前固定采用“简报标题 + 正式摘要带 + `StandardInlineState` + 表单主区”结构；不得再回流大标题 hero、双层说明卡或让表单继续下沉到首屏之后。
