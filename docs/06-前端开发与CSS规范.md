...
- `接入智维` 单主列表页默认结构固定为“面包屑 -> 主工作台卡片 -> 筛选/工具条/结果/分页”；`产品定义中心` 与 `设备台账` 标题层级必须保持同一字号、字重和留白，不得因为页面不同再长出第三层导航标题。
...
- `链路追踪台` 与 `数据校验台` 顶部工具条不得再放 `异常观测台 / 数据校验台 / 链路追踪台` 这类跨页功能菜单；跨页联动统一收口到行级动作、详情抽屉提示或来源上下文恢复，不再制造右上角功能菜单感。
