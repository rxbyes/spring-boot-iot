...
- `StandardWorkbenchRowActions` 当前负责把 `StandardRowActions + StandardActionLink + StandardActionMenu` 收口为同一层页级组合：
  - `table` 变体默认使用 `compact` 间距；当桌面表格中可见动作达到 `3` 个时（典型为“前 2 项直出 + 更多”），共享组件默认启用 `between` 等距分布；只有 `1~2` 个可见动作时继续使用共享紧凑分布，页面不得再显式传 `distribution` 覆写。
...
- 纳管列表页中的 `操作` 列必须显式声明 `class-name="standard-row-actions-column"`，并通过 `resolveWorkbenchActionColumnWidth`（兼容场景可继续用 `resolveAdaptiveActionColumnWidth`）按“直出动作数量 + 是否存在更多菜单 + 标签长度”计算列宽；桌面端当前同时固定采用共享宽度分层：双动作桌面列统一使用同一基线宽度，出现 `更多` 时自动提升到三段式基线。共享全局样式继续统一处理 fixed 列裁切、字体和末尾留白；不要再依赖默认 `.cell` ellipsis，让最后一个动作后面冒出圆点，也不要再写死 `200 / 220 / 288` 一类宽度。
- 详情抽屉优先采用“列表摘要秒开 + 完整详情后台静默补数”的体验；不得用红色错误提示表达正常刷新中的补数过程。
- 当表格操作列固定为 1~2 个轻操作时，必须通过 `StandardWorkbenchRowActions` 统一间距，并保持共享紧凑分布；当桌面端形成 `详情 / 编辑 / 更多` 这类 `3` 个可见动作的三段式组合时，才由共享组件自动启用等距分布。页面不得再显式传 `distribution`，也不要在 scoped CSS 里单独覆写 gap，或回退到页面私有 margin / gap hack。
...
- 列表页“操作”列当前统一以 `设备资产中心` 桌面端为三段式基线：表格行内动作必须复用 `StandardWorkbenchRowActions`；桌面 `table` 变体在仅有 `1~2` 个可见动作时保持共享紧凑分布，在 `详情 / 编辑 / 更多` 这类 `3` 个可见动作场景下自动启用共享等距分布，并继续搭配共享宽度分层。不得再为单页显式写 `distribution`、`gap="wide"`、页面私有列宽下限或 margin / gap hack。
...
- 复用 `standard-row-actions-column` 的桌面表格当前必须走共享自适应列宽策略：操作列宽度要按直出动作数量、是否存在 `更多` 菜单和实际标签长度自动收口，并继续落到共享宽度分层；双动作桌面列与“前 2 项直出 + 更多”的三段式桌面列必须保持同一套基线，不得再在页面里手写 `210 / 224 / 288` 一类超宽固定值，导致“操作”后方出现大块留白。
- `产品经营工作台` 头部当前固定为“唯一产品名 + 一句状态判断 + 去重元信息行 + pill 真页签”四段式；不得回流内部 kicker、头部设备指标带、第二组概况卡或重复产品身份。
- `经营总览` 当前固定为“1 个核心规模主舞台 + 3 个次级指标（在线覆盖 / 30 日活跃 / 平均在线） + 3 个摘要块（当前判断 / 契约基线 / 档案摘要）”；不得回流旧的 `趋势摘要 + 接入契约与产品档案` 双表格、正文右侧指标栏或头部重复设备规模。
...
- 若 `经营总览` 仍需表达变更约束，应优先内联到 `当前判断 / 契约基线 / 档案摘要` 三个摘要块中，不再单独回流 `维护与治理` 分层卡片。
