...
- 纳管列表页中的 `操作` 列必须显式声明 `class-name="standard-row-actions-column"`，并通过 `resolveWorkbenchActionColumnWidth`（兼容场景可继续用 `resolveAdaptiveActionColumnWidth`）按“直出动作数量 + 是否存在更多菜单 + 标签长度”计算列宽；桌面端当前继续采用共享宽度分层，但双动作桌面列不再保留过宽的旧 `144px` 下限，而是收口到更贴近内容的共享基线，避免 `详情 / 观测` 这类短文案在右侧留下过大空白；出现 `更多` 时仍自动提升到三段式基线。共享全局样式继续统一处理 fixed 列裁切、字体和末尾留白；不要再依赖默认 `.cell` ellipsis，让最后一个动作后面冒出圆点，也不要再写死 `200 / 220 / 288` 一类宽度。
