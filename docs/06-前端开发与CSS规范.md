...
- `StandardWorkbenchRowActions` 当前负责把 `StandardRowActions + StandardActionLink + StandardActionMenu` 收口为同一层页级组合：
  - `table` 变体默认使用 `compact` 间距，桌面列表页统一对齐产品定义中心的紧凑基线。
...
- 纳管列表页中的 `操作` 列必须显式声明 `class-name="standard-row-actions-column"`，并通过 `resolveWorkbenchActionColumnWidth`（兼容场景可继续用 `resolveAdaptiveActionColumnWidth`）按“直出动作数量 + 是否存在更多菜单 + 标签长度”计算列宽，由共享全局样式统一处理固定列裁切、字体和末尾留白；不要再依赖默认 `.cell` ellipsis，让最后一个动作后面冒出圆点，也不要再写死 `200 / 220 / 288` 一类宽度。
- 详情抽屉优先采用“列表摘要秒开 + 完整详情后台静默补数”的体验；不得用红色错误提示表达正常刷新中的补数过程。
- 当表格操作列固定为 1~2 个轻操作时，优先通过 `StandardWorkbenchRowActions` 或 `StandardRowActions variant="table" gap="compact"` 统一间距；不要在页面 scoped CSS 里再单独覆写“详情 / 编辑 / 更多”的 gap。
...
- 列表页“操作”列当前统一以 `产品定义中心` 为紧凑基线：表格行内动作必须复用 `StandardWorkbenchRowActions` 或 `StandardRowActions variant="table" gap="compact"`，不得再为单页显式写 `gap="wide"` 或放大动作之间的水平留白；若同一行同时存在 `详情 / 编辑 / 更多` 这类三段式动作，可在保持紧凑按钮 padding 的前提下追加 `distribution="between"` 做共享等距分布，不得回退到页面私有 margin / gap hack。
...
- 产品类详情当前至少拆分为“页眉数据带 / 趋势摘要 / 接入契约 / 产品档案”四个信息层；设备规模不再单独回流为首屏 hero 模块，趋势与档案字段也不得混写成同权重摘要卡墙。
- `/products` 的产品详情当前固定升级为 `60rem` 沉浸式详情工作台；继续复用 `StandardDetailDrawer`，但内部必须保持“统一页眉 + 趋势摘要与契约档案总览板”两层结构。趋势摘要固定采用 `2 行 x 3 列` 轻表格，接入契约与产品档案固定采用 `3 行 x 3 列` 轻表格，不得回退到 `Hero Stage / Governance Stage` 四段式。
...
- `产品定义中心` 详情首屏当前只允许保留一个总览板块：`趋势摘要与契约档案`；设备总量、在线设备、30 日活跃等核心读数必须上移到工作台头部右侧数据卡，`物模型设计器` 不得回流到详情头部动作。
- `产品经营工作台` 当前进一步固定为“轻头部 + 真页签 + 扁平主板块”的统一语法：头部只保留通用工作台标题、唯一产品名、去重后的 `productKey / 协议 / 节点 / 数据格式` 元信息和右侧核心数据卡；不得再回流状态判断句、工作台说明、厚 hero 背景或第二组概况卡。
- `产品经营工作台` 头部右侧当前固定承接 `经营总览` 的核心设备规模数据，并以紧凑卡片形式呈现；设备总量、在线设备、30 日活跃等高频读数优先放在页头右侧，不要再把同组核心指标留在总览正文里重复铺开。
- `经营总览` 当前固定采用“趋势摘要 + 接入契约与产品档案”双表格结构；趋势摘要必须保持 `2 行 x 3 列` 轻表格，接入契约与产品档案必须保持 `3 行 x 3 列` 轻表格，其中 `产品状态` 与 `产品说明` 同行，`产品说明` 占两列，不再保留独立设备规模舞台、左右双栏不均衡布局或正文重复指标卡。
...
- `产品经营工作台` 第二轮视觉升级当前固定采用“首长简报式”排版语法：优先通过通用工作台标题、唯一产品名、右侧数据卡、轻表格节奏和留白秩序建立气场，不依赖重装饰、重色块、重复标题或第二套品牌色。
- `经营总览 / 物模型治理 / 关联设备 / 编辑治理` 四页当前都必须满足“先定调，再读数，再落台账或正文”的阅读顺序；一级标题负责压场，二级章节负责定调，节内主舞台只允许一个视觉主焦点。
- `产品经营工作台` 头部当前继续强化为“通用工作台标题 + 唯一产品名 + 去重元信息条 + 右侧数据卡 + 真页签”五段式页眉；同一产品标识不得在抽屉标题、产品名、产品 Key 中重复出现两次及以上。
- `物模型治理` 当前固定要有 `当前判断` 句、候选正文导语和正式模型导语；`关联设备` 当前固定要有 `章节头 + 摘要横带 + 台账导语 + 台账主区`；`编辑治理` 当前固定要有 `章节头 + 变更上下文条 + 表单导语 + 表单主区`。若缺失这些导语层，视为又退回普通组件拼装页。
- `关联设备` 的指标区当前固定升级为统一摘要横带，`编辑治理` 当前固定升级为“变更上下文条 + 表单主区”；后续若再退回轻散列卡片、普通表单头或组件拼装页，视为样式回退。
- `经营总览` 当前继续执行极简文案规则：区块内优先保留标题、字段和值；趋势摘要、接入契约和产品档案必须收口为轻表格，重复解释、副说明、接入提示卡、状态胶囊、额外判断句、正文重复指标卡和独立治理区都应优先删除，不要把详情页重新做回“说明墙”。
...
- 若 `经营总览` 仍需表达变更约束，应优先内联到趋势摘要或契约档案表格中，不再单独回流 `维护与治理` 分层卡片。
