...
19. `IotAccessPageShell`：统一 `接入智维` 子页的轻量标题壳、顶部动作与紧凑状态条。
20. `IotAccessTabWorkspace`：统一 `接入智维` 子页的同页 Tab 工作区；默认页签必须是当前页面的最高频任务。
21. `IotAccessResultSection`：统一 `接入智维` 子页中台账之外的辅战区、结果分区和轻工具条头部语法。
...
- `接入智维` 一级模块当前统一采用“紧凑入口/状态条 + 主战区”的页面语法；总览页只保留最近使用、推荐处理顺序和全部能力，子页不得重复模块目录或能力墙。
- `产品定义中心`、`设备资产中心`、`链路验证中心`、`异常观测台`、`链路追踪台`、`数据校验台` 当前已进一步统一为“轻量页面壳 + 同页 Tab 工作区 + 主台账/主结果区 + 辅战区”的第二层编排；新增接入侧页面时先接这套壳，不再自起一套局部编排协议。
- `产品定义中心`、`设备资产中心` 顶部文案必须保持资产治理语气，主战区仍是台账和治理动作；`链路验证中心`、`异常观测台`、`链路追踪台`、`数据校验台` 顶部文案必须保持诊断语气，主战区直接进入验证、追踪或校验结果。
- `接入智维` 子页若存在两个及以上并列内容域，必须优先复用 `IotAccessPageShell + IotAccessTabWorkspace + IotAccessResultSection` 组织结构；不要再散写页面私有标题条、pill 切换和说明分区。
- `/system-log` 的系统异常模式可以使用接入智维诊断语气的紧凑状态条；`/audit-log` 的审计中心模式必须继续保持列表优先，不得复制接入侧诊断导流文案。
...
- `/file-debug` 继续保持“紧凑状态条 + 顶部查询入口 + 校验结果 + 原始响应”四段式工作台结构，不回退到散开的 Hero 表单卡、独立说明卡或重复概况卡。
- `deviceCode` 仍是数据校验台唯一查询入口；顶部查询区优先复用 `StandardWorkbenchPanel + StandardListFilterHeader`，状态反馈优先通过 `StandardInlineState` 表达。
- 文件快照、固件聚合和原始响应继续优先复用 `PanelCard`、`ResponsePanel` 等共享件；若顶部状态条已经承接数量与刷新状态，就不要再在结果区重复堆叠第二层概况卡。
