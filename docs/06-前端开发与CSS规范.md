...
- `/insight` 的指标编排当前采用“样板设备直配 + 设备类型模板 + 运行时属性识别 + 轻配置覆盖”四层口径：样板设备可固定指定核心指标；其他设备至少要按 `预警型 / 监测型 / 采集型` 差异化挑选核心监测值与状态值；额外状态参数如 `humidity / signal_4g` 若出现在属性快照中，应自动进入“系统自定义参数”和状态趋势请求；轻配置读取优先级固定为“设备 `metadataJson.objectInsight.customMetrics[]` > 产品 `metadataJson.objectInsight.customMetrics[]` > 前端内置注册表 > 运行时自动识别”，避免继续退化为只靠单个样板机写死展示。
