...
- `/message-trace` 详情抽屉当前固定采用客户视角收纳后的 `A 完整内容平衡版` 刊栏语法：`StandardDetailDrawer` 标题行承担轻刊头，但不得再重复展示 `消息类型 / TraceId / Topic` 标签；识别信息统一由正文中的 `消息概览 lead-sheet + 链路信息` 承接。正文完整保留 `消息概览 lead-sheet + 链路信息 + Payload 对照 + 处理时间线` 四个章节，首屏视觉必须以附件终版的大标题 lead-sheet 和右侧指标列为准，不得回退到设备详情 hero 语法，也不得重新长出说明墙、排查建议或跨页导流段落。链路信息不得再回退成一整块无分组台账，至少要拆成 `链路标识` 与 `接入上下文` 两组阅读单元；其中 `链路标识` 固定收口为 `日志 ID / 创建时间 / TraceId`，`Topic` 固定保留在 `接入上下文` 组内。`消息概览` 右侧的 `上报时间 / Topic 节点 / 产品标识 / 日志 ID` 必须按 `2 x 2` 指标块对齐，默认单行截断，不要让长值把首屏节奏打散；显式带 `UTC / offset` 的上报时间必须统一换算为 `Asia/Shanghai` 后展示，无时区的历史时间串则继续按原值显示，避免二次偏移。消息类型若命中运行态 `status`，前端展示必须映射为 `状态上报`，不得把协议原词直接暴露给终端用户。`Payload 对照` 必须同时保留 `原始 Payload / 解密后明文 / 解析结果` 三个阅读单元，但展示形态固定为三条纵向折叠板块而不是三列常显卡片；每个板块默认收起、支持按需展开与复制，且不再附带“有内容 / 暂无内容”一类辅助状态 chip 干扰判断。原始 Payload 来自消息日志，解密明文与解析结果优先来自详情恢复接口重建结果，`PROTOCOL_DECODE.summary` 仅作为补充回退；若详情恢复接口返回空字符串、空对象或等价空值，前端必须继续基于原始 Payload 做兜底，不得把“有原始报文”的链路渲染成明文/解析双空。处理时间线必须继续作为独立全文宽章节承接固定 Pipeline 复盘，但默认只展示摘要和降级提示，展开后再进入完整时间线；详情抽屉的配色也必须继续复用系统浅色控制台 token，不要再把 Payload、时间线单独做成重渐变私有风格。时间线过期时不得把 payload 对照回退成只剩原始 Payload，也不得继续显示“无法恢复”误导文案。
