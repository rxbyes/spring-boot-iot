...
- 时序表初始化：仓库当前已提供可直接执行的 `sql/init-tdengine.sql`，用于初始化 `iot_device_telemetry_point` 兼容表和 telemetry v2 raw stable（`iot_raw_measure_point`、`iot_raw_status_point`、`iot_raw_event_point`）；应用启动阶段仍保留 `CREATE IF NOT EXISTS` 级别 schema 补齐能力，用于历史环境兜底，不自动回灌历史数据。真实环境 legacy stable 继续由环境侧治理，代码不负责补 stable DDL。
...
- `sql/init-tdengine.sql`：TDengine 初始化脚本。负责建立 `iot` 时序库、`iot_device_telemetry_point` 兼容表和 telemetry v2 raw stable，并在脚本内直接说明表作用、字段含义和 child table 生成规则。
...
- TDengine 兼容表 `iot_device_telemetry_point` 与 telemetry v2 raw stable 当前既可通过 `sql/init-tdengine.sql` 直接初始化，也保留运行时自动补齐能力；child table 仍按设备运行时动态创建。
...
- `/products` 抽屉内当前默认治理流程为“手动提炼 -> 正式模型 -> 确认写库”；手动提炼与运行期候选都不新增候选草案表，候选确认后才写入 `iot_product_model`。
- 手动提炼当前只服务于当前选中的产品，且单次只支持解析 `1` 个设备样本 JSON；`business / status / other` 三类样本都会递归下钻到标量叶子后生成属性候选。
...
- 当前手动提炼会对 `sampleType=other` 的字段默认标记 `needsReview`；运行期候选仍会对 `codex_verify_*`、`singal_*` 等命名漂移字段标记 `needsReview`，但都不会改动库结构；是否入正式契约完全由确认接口决定。
- 当前真实环境 `iot_command_record` 仍可能停留在旧 schema，缺少 `service_identifier` 等字段；当字段未对齐时，服务候选提炼保持“空结果 + 提示”策略，不做 schema 回写和补列。
- 手动提炼当前仅自动生成 `property` 候选；`event / service` 仍由正式模型人工维护，不在 `manual-extract` 中自动补写。
...
- TDengine 兼容表 `iot_device_telemetry_point` 与 telemetry v2 raw stable 已收口到 `sql/init-tdengine.sql`，同时 `spring-boot-iot-telemetry` 仍保留运行时自动补齐能力，避免历史环境因缺表阻断。
