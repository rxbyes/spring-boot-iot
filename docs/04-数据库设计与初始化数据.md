...
- `risk_level`：事件风险等级，当前统一使用 `red / orange / yellow / blue` 四色内码；历史 `critical / warning / info` 应通过初始化脚本或 `scripts/run-real-env-schema-sync.py` 迁移为新值。
...
- `risk_level`：适用风险等级，当前统一使用 `risk_level` 字典返回的 `red / orange / yellow / blue` 四色内码；自动闭环匹配仍兼容历史库存量 `critical / warning / info`。
  - `description`：预案说明。
...
- 历史库若仍保留 `critical / warning / info` 风险等级，执行 `sql/init-data.sql` 或 `python scripts/run-real-env-schema-sync.py` 时会同步把 `risk_level` 字典、`risk_point`、`emergency_plan`、`iot_event_record` 迁移到 `red / orange / yellow / blue` 四色内码。
