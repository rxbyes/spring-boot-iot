...
- `sql/init.sql`：全量 DDL 基线。脚本会先 `CREATE DATABASE rm_iot`、`USE rm_iot`、`SET NAMES utf8mb4`，再按“先删表、再重建”的顺序创建当前基线 `67` 张 active MySQL 表；当前不再创建 MySQL 兼容视图。
...
| --- | --- | --- | --- | --- | --- |
| MySQL active 表 | 67 | 是 | 是 | 是 | 当前业务主链路默认初始化对象 |
...
- `iot_message_log` 作为消息日志唯一物理真表，承接链路追踪、验收证据和治理回放读写。
...
| 运行日志 | `iot_message_log`（唯一物理真表）、`sys_business_event_log`、`sys_observability_span_log` | `trace_id`、`topic`、`message_type`、`event_code`、`span_type`、`tenant_id` | [03-接口规范与接口清单.md](./03-接口规范与接口清单.md)、[11-可观测性、日志追踪与消息通知治理.md](./11-可观测性、日志追踪与消息通知治理.md) |
