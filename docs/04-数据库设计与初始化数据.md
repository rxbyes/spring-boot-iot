...
- `sql/init.sql`：全量 DDL 基线。脚本会先 `CREATE DATABASE rm_iot`、`USE rm_iot`、`SET NAMES utf8mb4`，再按“先删视图、后删表、再重建”的顺序创建当前基线 `55` 张 MySQL 表，并在末尾补齐 `iot_message_log` 兼容视图。
...
| --- | --- | --- | --- | --- | --- |
| MySQL active 表 | 55 | 是 | 是 | 是 | 当前业务主链路默认初始化对象 |
...
- 快照：冻结某次发布或审批时刻的结构真相，例如 `iot_product_contract_release_snapshot`、`iot_product_metric_resolver_snapshot`。
