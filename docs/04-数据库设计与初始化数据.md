...
- `2026-04-09` 起，`sql/init-data.sql` 额外内置平台治理页面菜单 `93003022 / system:governance-approval / /governance-approval`，用于显式暴露治理审批台。
- 字段：
...
- `对象洞察台` 已挂入 `风险运营`
- `自动化工场` 已挂入 `质量工场`
- `平台治理` 当前已补齐 `治理审批台` 页面菜单，默认回填 `MANAGEMENT_STAFF`、`OPS_STAFF`、`DEVELOPER` 与超管角色授权；共享环境若缺 `/governance-approval`，需先对齐最新 `sql/init-data.sql` 后再复验审批链路。
