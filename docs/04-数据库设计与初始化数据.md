...
- 退场说明：旧对象名 `iot_device_message_log` 已不再进入 registry、初始化脚本、schema sync manifest 或运行时 bootstrap；历史共享库中若仍存在同名表或视图，由 `python scripts/run-real-env-schema-sync.py` 迁移旧表数据后删除，或直接删除旧兼容视图。
