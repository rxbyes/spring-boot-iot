...
- `risk_level`：事件风险等级，当前运行态统一按 `red / orange / yellow / blue` 四色内码理解；共享环境若仍保留历史 `critical / warning / info`，查询、展示与自动闭环匹配都需兼容，不再把批量迁移作为前置条件。
...
- 作用：风险对象主表，定义风险点基础档案。
- 字段：
  - `id` / `risk_point_code` / `risk_point_name`：主键、风险点编号和名称；其中 `risk_point_code` 当前在新增时由服务端根据所属组织、风险点名称和风险等级自动生成，且会避开同租户内已软删除历史记录占用的编号；由于唯一索引按 `(tenant_id, risk_point_code)` 生效，更新时保留原值。
  ...
  - `risk_level`：风险等级，当前统一绑定平台治理 `risk_level` 字典的 `red / orange / yellow / blue` 四色内码。
...
- 字段：
  ...
  - `project_risk_level`：Excel 原始项目风险等级；当前初始化源数据大多为空，因此主表运行态风险等级仍统一落 `blue`。
...
- 字段：
  ...
  - `expression`：阈值表达式。
  - `alarm_level` 当前查询与页面展示兼容 `critical / warning / info`、`red / orange / yellow / blue` 以及运行态 `high / medium / low` 混合库存；页面编辑态统一归一为 `critical / warning / info` 三档。
  - `duration`：持续时间阈值（秒）。
...
- 字段：
  ...
  - `risk_level`：适用风险等级，当前统一使用 `risk_level` 字典返回的 `red / orange / yellow / blue` 四色内码；自动闭环匹配、列表查询、页面展示与编辑回显仍兼容历史库存量 `critical / warning / info`。
  - `description`：预案说明。
...
- 历史库若仍保留 `critical / warning / info` 风险等级，可通过 `sql/init-data.sql` 或 `python scripts/run-real-env-schema-sync.py` 逐步对齐到 `red / orange / yellow / blue` 四色内码；当前系统已在查询、展示、编辑回显与自动闭环匹配侧兼容旧值，不再把批量迁移作为共享环境可用性的前提。
