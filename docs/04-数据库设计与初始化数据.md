...
- 时序表初始化：`iot_device_telemetry_point` 由 `spring-boot-iot-telemetry` 在 TDengine `slave_1` 数据源上启动时执行 `CREATE TABLE IF NOT EXISTS` 自动补齐，不额外依赖 MySQL 初始化脚本。
...
- `iot_command_record`
- `iot_device_telemetry_point`（TDengine）
...
- `iot_device_telemetry_point` 按“一条标准化属性点一行”落 TDengine 历史时序，当前只持久化 `properties`，`reply` / 文件载荷 / 空属性消息不写入该表。
...
#### 2.2.3 `iot_device_telemetry_point`（TDengine）字段口径

| 字段 | 含义 | 当前写入口径 |
|---|---|---|
| `ts` | 时序时间 | 优先使用报文标准化后的 `timestamp`，缺失时回退到服务端当前时间 |
...
补充口径：
- 当前采用单一通用表，不做按产品动态宽表或按设备 supertable。
- `/api/telemetry/latest` 在 `tdengine` 模式下按 `device_id + metric_code + MAX(ts)` 聚合该表中的最新点。
...
| 运行日志 | `iot_message_log` / `iot_device_message_log` | `trace_id`、`topic`、`message_type`、`tenant_id` | [03-接口规范与接口清单.md](./03-接口规范与接口清单.md)、[11-可观测性、日志追踪与消息通知治理.md](./11-可观测性、日志追踪与消息通知治理.md) |
| 历史时序 | `iot_device_telemetry_point`（TDengine） | `ts`、`device_id`、`metric_code`、`trace_id`、`value_*` | 本文 `2.2.3`、[03-接口规范与接口清单.md](./03-接口规范与接口清单.md) |
