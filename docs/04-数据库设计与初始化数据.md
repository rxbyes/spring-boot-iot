...
- 历史环境若需对齐当前代码，优先使用最新 `sql/init.sql` / `sql/init-data.sql` / `sql/init-tdengine.sql` 重建或初始化；无法重建时，再人工比对脚本结构或执行 `python scripts/run-real-env-schema-sync.py`。
...
- 风险治理相关字典当前已内置 `risk_point_level`（一级/二级/三级风险点）、`alarm_level`（红/橙/黄/蓝）与 `risk_level`（运行态风险红/橙/黄/蓝）。
- 通知渠道样例：
