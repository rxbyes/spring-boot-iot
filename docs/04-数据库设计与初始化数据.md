...
- telemetry v2 latest 投影表：`iot_device_metric_latest` 由升级脚本 `sql/upgrade/20260327_phase5_telemetry_v2_latest_projection.sql` 创建，当前不进入 `sql/init.sql`。
- 无效上报最新态表：`iot_device_invalid_report_state` 已进入 `sql/init.sql`；历史共享库需执行 `sql/upgrade/20260327_phase5_mqtt_invalid_report_state.sql` 补齐。

### 1.1 `sql/` 目录脚本分类

- `sql/init.sql`：全量 DDL 基线。脚本会先 `CREATE DATABASE rm_iot`、`USE rm_iot`、`SET NAMES utf8mb4`，再按“先删视图、后删表、再重建”的顺序创建当前基线 `34` 张 MySQL 表。
- `sql/init-data.sql`：全量初始化样例数据。主要采用 `INSERT ... ON DUPLICATE KEY UPDATE`、变量查询和幂等删除重建关联数据的方式，适合在同一环境重复执行。
- `sql/upgrade/*.sql`：增量升级脚本。用途覆盖 schema 对齐、菜单/权限回填、分页索引、系统内容治理、接入失败治理、时序 latest 投影补齐，以及少量按真实设备场景定制的数据初始化。
- 当前目录中最常见的语句模式包括：
  - `CREATE TABLE IF NOT EXISTS`：对历史库补齐缺失表。
  - `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`：幂等补齐字段。
  - `CREATE INDEX` / `ALTER TABLE ... ADD INDEX`：为分页、检索、治理看板补齐索引。
  - `INSERT ... ON DUPLICATE KEY UPDATE`：幂等写入菜单、角色、样例数据和治理种子。
  - `PREPARE + EXECUTE`：根据 `information_schema` 动态判断目标环境是否已具备某字段/索引，再执行兼容 SQL。

### 1.2 当前脚本边界说明

- `sql/init.sql` 是当前唯一全量建库入口，默认与 [README.md](../README.md) 和 [07-部署运行与配置说明.md](./07-部署运行与配置说明.md) 保持一致。
- `sql/upgrade` 中既有通用 schema migration，也有“只适用于特定历史库或特定设备场景”的脚本；执行前必须先看本节和本文第 `5` 节说明。
- `sql/upgrade/sys_region.sql` 是历史扩展区域库全量种子，不是当前 `sql/init.sql` 的直接补充脚本。
  - 它依赖扩展版 `sys_region` 字段，例如 `region_path`、`boundary`、`responsible_user`、`phone`。
  - 当前 `sql/init.sql` 中的 `sys_region` 基线只保留 `region_name`、`region_code`、`parent_id`、`region_type`、`longitude`、`latitude`、`status`、`sort_no`、`remark` 等字段，因此不能直接把 `sys_region.sql` 当作纯基线脚本无条件执行。
...
说明：
- 以下字段口径以 `sql/init.sql` 当前基线为准；若历史共享库只执行了部分 `sql/upgrade/*.sql`，实际字段集合可能只覆盖子集。
...
- 字段：
  ...
  - `remark`：补充说明。
  - `create_by/create_time/update_by/update_time/deleted`：审计字段和软删除标记。
- 补充说明：`sql/upgrade/sys_region.sql` 使用的是历史扩展版字段集，额外包含 `region_path`、`boundary`、`responsible_user`、`phone` 等列，不等同于当前 `init.sql` 基线。
...
- `/products` 抽屉内的“候选提炼”当前同样不新增候选草案表；属性 / 事件 / 服务候选由 `iot_device_property`、`iot_device_message_log` 与 `iot_command_record` 实时推导，确认后才写入 `iot_product_model`。
...
- 当前候选提炼会对 `codex_verify_*`、`singal_*` 等命名漂移字段标记 `needsReview`，但不会改动库结构；是否入正式契约完全由确认接口决定。
...
- raw 列当前固定包含 `ts`、`metric_id`、`reported_at`、`ingested_at`、`value_double`、`value_long`、`value_bool`、`value_text`、`quality_code`、`alarm_flag`、`trace_id`、`session_id`、`source_message_type`；tags 固定包含 `tenant_id`、`device_id`、`product_id`、`sensor_group`、`location_code`、`risk_point_id`。
- telemetry v2 foundation 当前只落 raw 主写、latest MySQL 投影和 legacy mirror 钩子；小时 / 天级 aggregate stable 尚未进入默认运行口径。
...
说明：
- 以下说明覆盖 `sql/init.sql` 中的 IoT MySQL 表，以及 `sql/upgrade` 补齐的 `iot_message_log` 兼容视图、`iot_device_metric_latest` latest 投影表。
- 其中 `iot_product`、`iot_product_model`、`iot_device_metric_latest`、`iot_device_telemetry_point` 已在前文有专项字段说明，下面侧重补全逐表用途与字段分组。
...
- 字段：
  ...
  - `manufacturer` / `description` / `remark`：厂商与说明信息。
  - `status`：产品启停状态。
  - `create_by/create_time/update_by/update_time/deleted`：审计字段和软删除标记。
- 详细治理口径见本文 `2.2.1`。
...
- 字段：
  - `id` / `tenant_id` / `product_id`：设备主键、租户范围和所属产品。
...
- 字段：
  - 当前不在 `sql/init.sql` 中建表，而是由 `spring-boot-iot-telemetry` 在 TDengine 侧启动时自动执行 `CREATE TABLE IF NOT EXISTS`。
...
- 字段：
  ...
  - `alarm_title` / `alarm_type` / `alarm_level`：告警标题、类型和等级。
...
- 字段：
  ...
  - `event_title`：事件标题。
  - `alarm_id` / `alarm_code` / `alarm_level`：来源告警及其等级。
  - `risk_level`：事件风险等级。
...
- 作用：风险对象主表，定义风险点基础档案。
- 字段：
  - `id` / `risk_point_code` / `risk_point_name`：主键、风险点编号和名称。
  - `region_id` / `region_name`：所属区域。
...
- 目录中的升级脚本并不都是“通用建表脚本”，还包括菜单授权回填、性能索引补齐、系统内容演示数据、接入失败治理和按特定设备场景定制的数据脚本。
- 多数脚本采用幂等写法，例如 `CREATE TABLE IF NOT EXISTS`、`ALTER TABLE ... ADD COLUMN IF NOT EXISTS`、`INSERT ... ON DUPLICATE KEY UPDATE` 或基于 `information_schema` 的动态 DDL。
- 涉及历史库的菜单脚本存在时间演进关系：较早的菜单基线脚本用于旧工作台结构，后续脚本会对其进行重构或覆盖。
- `20260320_phase4_deep_displacement_sub_devices_bootstrap.sql` 和 `sys_region.sql` 主要是数据脚本，不应与通用 schema migration 等同看待。

### 5.2 脚本索引

1. `20260316_iot_message_log_view.sql`：创建 `iot_message_log` 兼容视图，把对外统一命名映射到物理表 `iot_device_message_log`。
2. `20260316_phase4_real_env_schema_alignment.sql`：Phase 4 真实环境结构对齐脚本。
   - 会补齐 `iot_command_record`、`iot_alarm_record`、`iot_event_record`、`iot_event_work_order`、`risk_point`、`risk_point_device`、`rule_definition`、`linkage_rule`、`emergency_plan`、`sys_organization`、`sys_role`、`sys_dict`、`sys_dict_item`、`sys_notification_channel`、`sys_audit_log` 等表。
   - 同时通过 `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` 把历史库缺失的兼容字段补齐到现有表。
3. `20260316_phase4_task3_risk_monitoring_schema_sync.sql`：风险监测拆分脚本。
   - 重点补 `risk_point.create_by/update_by`，并补齐或创建 `risk_point_device` 绑定表。
   - 适用于 Phase 4 早期只执行了部分监测库升级的环境。
4. `20260316_phase4_task10_dynamic_menu_auth.sql`：旧版动态菜单与授权基线。
   - 会补齐旧三工作台时代的菜单树、超管授权和部分角色菜单。
   - 若环境已经执行 `20260319_phase5_workspace_menu_refactor.sql`，应把本脚本视为历史菜单路径，不再作为最新工作台布局依据。
5. `20260317_phase4_menu_button_permission_backfill.sql`：菜单管理按钮权限回填。
   - 仅补齐 `system:menu:add/update/delete` 三个按钮及 `SUPER_ADMIN` 绑定。
   - 适用于历史库菜单页面存在、但按钮权限未落到真实超管角色的场景。
6. `20260317_phase4_system_governance_paging_indexes.sql`：系统治理分页和检索索引补齐。
   - 重点为 `sys_user`、`sys_role`、`sys_menu`、`sys_organization`、`sys_region`、`sys_dict`、`sys_notification_channel`、`sys_audit_log` 补索引。
   - 主要解决树表懒加载、分页和按 `trace_id/device_code` 检索日志的性能问题。
7. `20260317_phase5_automation_test_center_menu.sql`：早期“自动化测试中心”菜单脚本。
   - 会在当时的 `平台治理` 下挂 `system:automation-test`。
   - 后续已被 `20260319_phase5_workspace_menu_refactor.sql` 的“质量工场/自动化工场”布局取代。
8. `20260319_phase5_demo_text_repair.sql`：历史演示账号、角色名称和按钮标题乱码修复脚本。
   - 使用 `SET NAMES utf8mb4` 和 `CONVERT(0x... USING utf8mb4)` 幂等回写中文。
   - 适用于 Windows 终端或旧脚本执行时把中文写成 `????` 的共享库。
9. `20260319_phase5_device_asset_center.sql`：设备资产中心按钮与授权补齐。
   - 会补齐 `iot:devices:add/update/delete/export/import/replace` 六个按钮权限。
   - 同时按 `role_code` 回填业务、管理、运维、开发、超管五类角色授权。
10. `20260319_phase5_product_definition_center.sql`：产品定义中心页面和按钮补齐。
    - 会补齐 `/products` 页面菜单以及 `iot:products:add/update/delete/export` 四个按钮权限。
    - 同时按真实角色 ID 回填角色菜单。
11. `20260319_phase5_workspace_menu_refactor.sql`：五大工作台重构脚本。
    - 把一级导航统一重构为 `接入智维 / 风险运营 / 风险策略 / 平台治理 / 质量工场`。
    - 会整体重建页面菜单、角色授权范围和超管全量菜单绑定，是当前工作台布局的主升级脚本。
12. `20260320_phase4_deep_displacement_sub_devices_bootstrap.sql`：南方测绘深部位移基准站子设备初始化脚本。
    - 不是通用 schema 脚本，而是针对基准站 `SK00FB0D1310195` 自动补齐 `dispsX/dispsY` 物模型、`8` 个子设备和对应风险点种子数据。
    - 仅适用于真实环境已有该基准站设备的场景。
13. `20260320_phase5_product_stats_indexes.sql`：产品定义中心统计索引补齐。
    - 为 `iot_device` 新增 `idx_device_deleted_product_stats (deleted, product_id, last_report_time, online_status)`。
    - 解决 `/products` 分页时的关联设备数、在线数和最近上报时间聚合扫描过大的问题。
14. `20260321_phase5_device_online_session.sql`：设备在线会话表建表脚本。
    - 新增 `iot_device_online_session`，并补齐按设备活跃会话和按产品时间段统计所需索引。
    - 产品详情页的在线时长统计依赖这张表。
15. `20260321_phase5_in_app_message_help_docs.sql`：系统内容基础表和演示数据脚本。
    - 新建 `sys_in_app_message`、`sys_in_app_message_read`、`sys_help_document`。
    - 同时写入首批 `4` 条站内消息样例和 `13` 条帮助文档样例。
    - 对“历史库已有 `sys_in_app_message` 但缺 `dedup_key`”的场景也做了兼容补列。
16. `20260321_phase5_system_content_menu_governance.sql`：系统内容治理菜单脚本。
    - 在 `平台治理` 下补齐 `站内消息`、`帮助文档` 页面，以及 `system:in-app-message:add/update/delete`、`system:help-doc:add/update/delete` 六个按钮权限。
    - 角色授权只依赖精简版 `sys_role_menu` 基础字段集，兼容历史库。
17. `20260322_phase5_device_access_error_archive.sql`：设备接入失败归档表建表脚本。
    - 新建 `iot_device_access_error_log`，用于持久化 MQTT / `$dp` 前置校验失败时的载荷和错误摘要。
18. `20260322_phase5_device_access_error_contract_snapshot.sql`：接入失败归档契约快照补列脚本。
    - 给 `iot_device_access_error_log` 新增 `contract_snapshot`，用于固化“预期契约 vs 已知契约 vs 当前入口协议”。
19. `20260322_phase5_notification_center_followup.sql`：通知中心治理增强脚本。
    - 给 `sys_in_app_message` 补 `dedup_key` 列和相关索引。
    - 把历史来源值统一回填为 `manual` / `governance` 等当前标准来源，并重算去重键。
20. `20260322_phase5_notification_channel_bridge.sql`：站内消息桥接日志表建表脚本。
    - 新建 `sys_in_app_message_bridge_log`，记录“每条消息 + 每个渠道 + 每个场景”的最新桥接状态。
21. `20260322_phase5_notification_channel_bridge_attempt_log.sql`：桥接尝试明细表建表脚本。
    - 新建 `sys_in_app_message_bridge_attempt_log`，记录每次桥接调用的返回结果和尝试序号。
22. `20260324_phase5_tdengine_mapping_gap_draft.sql`：TDengine legacy 映射治理草案。
    - 面向 `south_rtu`、`south_multi_displacement`、`south_gnss_monitor`、`south_deep_displacement` 四类 live 产品预览缺口。
    - 默认 `@apply_changes = 0`，只输出建议，不直接把未经核验的 `stable/column` 落库。
23. `20260327_phase5_mqtt_invalid_report_state.sql`：无效 MQTT 上报最新态表建表脚本。
    - 新建 `iot_device_invalid_report_state`，并补唯一键 `uk_invalid_report_state_governance_key` 以及按设备/原因检索索引。
24. `20260327_phase5_telemetry_v2_latest_projection.sql`：telemetry v2 latest 投影表建表脚本。
    - 新建 `iot_device_metric_latest`，以 `(tenant_id, device_id, metric_id)` 做唯一键。
    - 用于 `/api/telemetry/latest` 在 v2 读路由下的默认回源。
25. `sys_region.sql`：区域全量种子脚本。
    - 内容是大体量的行政区/区域树插入语句，属于历史扩展区域库初始化资产。
    - 该脚本依赖扩展版 `sys_region` 字段集，不能在当前纯 `init.sql` 基线下无条件执行。
...
- 若准备执行 `sql/upgrade/sys_region.sql`，必须先确认目标环境的 `sys_region` 是否仍保留 `region_path`、`boundary`、`responsible_user`、`phone` 等历史扩展字段；当前 `init.sql` 基线不直接兼容这份脚本。
