...
- `sql/init-data.sql`：全量初始化样例数据。主要采用 `INSERT ... ON DUPLICATE KEY UPDATE`、变量查询和幂等删除重建关联数据的方式，适合在同一环境重复执行；当前已显式为验收设备样例写入 `iot_device.org_id / org_name`，保持设备主档与已绑定风险点组织口径一致；`2026-04-18` 起还继续回填 `/device-onboarding` 菜单与管理/运维/开发角色默认授权。
...
- 主真相：直接承载业务当前态与主链路写入，例如 `iot_product`、`iot_product_model`、`iot_device_onboarding_case`、`iot_device`、`risk_point`、`sys_governance_approval_order`、`iot_protocol_family_definition`、`iot_protocol_decrypt_profile`、`iot_protocol_template_definition`。
...
- `iot_device_onboarding_case` 当前承载“无代码接入案例”主真相：用最小字段集显式记录某类设备接入所需的协议治理、产品治理和正式合同发布事实。服务端会统一把 `protocol_family_code / decrypt_profile_code / protocol_template_code / product_id / release_batch_id` 派生成 `current_step / status / blocker_summary_json`，用于 `/device-onboarding` 工作台回答“卡在哪、下一步去哪”；该表当前只服务第一阶段 intake 与卡点收口，不替代产品合同、协议安全或运行时接入真相。
- 若执行 `scripts/reset-product-governance-data.py` 清理产品治理派生数据，`iot_product` 主档继续保留，但 `metadata_json` 中已有的对象洞察/产品治理扩展字段会被整体清空；`iot_device_onboarding_case` 不会删行，只会回退正式合同发布事实，使案例重新停在 `CONTRACT_RELEASE + IN_PROGRESS`。
...
- `iot_message_log` 兼容视图、`iot_device_metric_latest`、`iot_device_online_session`、`iot_device_access_error_log`、`iot_device_invalid_report_state`、`iot_device_onboarding_case` 等当前正式交付结构，都已直接进入 `sql/init.sql`。
