...
> 权威级别：一级权威。
> 上游来源：`sql/init.sql`、`sql/init-data.sql`、实体与 mapper 实现。
> 下游消费：环境初始化、历史库对齐、帮助中心治理、验收核表。
> 变更触发条件：表结构变化、初始化样例变化、历史库对齐策略变化、命名兼容变化。
> 更新时间：2026-03-31

本文件以 `sql/init.sql` 与 `sql/init-data.sql` 为事实基础。
...
- 历史库对齐：以最新 `sql/init.sql`、`sql/init-data.sql` 为准重建或人工比对；仓库不再提供独立增量 SQL 目录。
- 时序表初始化：`spring-boot-iot-telemetry` 会在 TDengine `slave_1` 数据源启动阶段自动补齐 `iot_device_telemetry_point` 兼容表和 telemetry v2 raw stable（`iot_raw_measure_point`、`iot_raw_status_point`、`iot_raw_event_point`）；真实环境 legacy stable 继续由环境侧治理，代码不负责补 stable DDL。
...
- 仓库当前不再保留独立增量 SQL 文件。原增量脚本中已进入正式交付基线的 DDL、索引、兼容视图和样例数据，已经并回 `sql/init.sql` / `sql/init-data.sql`；只适用于历史共享环境的临时脚本不再随仓库存续。
- 当前目录中最常见的语句模式包括：
...
- `sql/init-data.sql` 是当前唯一初始化数据入口，菜单、角色授权、系统内容样例和通知渠道基线都直接以内收方式维护。
- 历史环境若需对齐当前代码，优先使用最新 `sql/init.sql` / `sql/init-data.sql` 重建；无法重建时，再人工比对 `init.sql` 结构或执行 `python scripts/run-real-env-schema-sync.py`。
- TDengine 兼容表 `iot_device_telemetry_point` 与 telemetry v2 raw stable 仍由运行时自动补齐；仓库不再保留额外 TDengine SQL 文件。
...
说明：
- 以下字段口径以 `sql/init.sql` 当前基线为准；若历史共享库尚未按当前初始化基线对齐，实际字段集合可能存在漂移。
...
- 仓库 SQL 入口当前只保留 `sql/init.sql` 与 `sql/init-data.sql` 两个文件。
- 原独立增量 SQL 目录中的正式基线 DDL、索引、兼容视图和样例数据，已经并回这两个初始化文件。
...
- TDengine 兼容表 `iot_device_telemetry_point` 与 telemetry v2 raw stable 仍由 `spring-boot-iot-telemetry` 在运行时自动补齐；仓库不再保留额外 TDengine SQL 文件。

### 5.2 历史库治理策略

- 优先策略：直接以最新 `sql/init.sql`、`sql/init-data.sql` 重建目标环境。
