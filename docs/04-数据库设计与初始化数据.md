...
- `2026-04-10` 起，产品合同固定复核人策略新增落库 `sys_governance_approval_policy`，当前仍以 `sql/init.sql` + `sql/init-data.sql` 为准；若历史环境只执行了 schema sync、未重放初始化 SQL，则仍需额外对齐该表和固定复核人种子数据。
...
- `init-data.sql` 当前还会同步写入 2 条全局治理审批策略：`PRODUCT_CONTRACT_RELEASE_APPLY -> governance_reviewer`、`PRODUCT_CONTRACT_ROLLBACK -> governance_reviewer`，用于 `/products` 合同发布与回滚自动解析固定复核人。
