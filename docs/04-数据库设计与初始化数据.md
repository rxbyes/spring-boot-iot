...
13. `20260321_phase5_in_app_message_help_docs.sql`：补齐站内消息、消息已读、帮助文档三张系统治理表，并同步写入最小演示数据，供右上角 `通知中心 / 帮助中心` 后续直接切换真实后端数据源。
14. `20260321_phase5_system_content_menu_governance.sql`：补齐平台治理下的 `站内消息 / 帮助文档` 页面菜单、按钮权限和管理/超管角色授权，供 `/in-app-message` 与 `/help-doc` 前端编排页直接挂载到真实菜单树。
