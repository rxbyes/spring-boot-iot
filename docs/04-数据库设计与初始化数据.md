...
- TDengine 兼容表 `iot_device_telemetry_point` 仍由运行时自动补齐；仓库不再保留额外 TDengine SQL 文件。
...
- TDengine 兼容表 `iot_device_telemetry_point` 仍由 `spring-boot-iot-telemetry` 在运行时自动补齐；仓库不再保留额外 TDengine SQL 文件。
