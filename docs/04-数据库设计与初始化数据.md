...
- `sql/init.sql`：全量 DDL 基线。脚本会先 `CREATE DATABASE rm_iot`、`USE rm_iot`、`SET NAMES utf8mb4`，再按“先删视图、后删表、再重建”的顺序创建当前基线 `56` 张 MySQL 表，并在末尾补齐 `iot_message_log` 兼容视图。
...
- `2026-04-11` 起，`python scripts/run-real-env-schema-sync.py` 会同步对齐产品合同固定复核人种子：若历史环境缺少 `governance_reviewer` 账号、缺少其 `SUPER_ADMIN` 角色绑定，或缺少 `PRODUCT_CONTRACT_RELEASE_APPLY / PRODUCT_CONTRACT_ROLLBACK / VENDOR_MAPPING_RULE_PUBLISH / VENDOR_MAPPING_RULE_ROLLBACK` 四条全局固定复核策略，脚本会一并补齐，避免 `/products` 提交合同或映射规则治理审批时继续因旧库缺表/缺种子数据报错。
...
| --- | --- | --- | --- | --- | --- |
| MySQL active 表 | 56 | 是 | 是 | 是 | 当前业务主链路默认初始化对象 |
...
- 主真相：直接承载业务当前态与主链路写入，例如 `iot_product`、`iot_product_model`、`iot_device`、`risk_point`、`sys_governance_approval_order`。
- 快照：冻结某次发布或审批时刻的结构真相，例如 `iot_vendor_metric_mapping_rule_snapshot`、`iot_product_contract_release_snapshot`、`iot_product_metric_resolver_snapshot`。
...
- 字段：
  ...
  - `action_code`：治理动作编码；当前初始化基线已内置 `PRODUCT_CONTRACT_RELEASE_APPLY`、`PRODUCT_CONTRACT_ROLLBACK`、`VENDOR_MAPPING_RULE_PUBLISH` 与 `VENDOR_MAPPING_RULE_ROLLBACK`。
...
- `init-data.sql` 与 `scripts/run-real-env-schema-sync.py` 当前都会同步写入 / 对齐 4 条全局治理审批策略：`PRODUCT_CONTRACT_RELEASE_APPLY -> governance_reviewer`、`PRODUCT_CONTRACT_ROLLBACK -> governance_reviewer`、`VENDOR_MAPPING_RULE_PUBLISH -> governance_reviewer`、`VENDOR_MAPPING_RULE_ROLLBACK -> governance_reviewer`，用于 `/products` 合同发布 / 回滚及厂商字段映射规则发布 / 回滚自动解析固定复核人。
...
- `python scripts/run-real-env-schema-sync.py` 当前还会对齐治理审批固定复核人基线：自动补 `sys_governance_approval_policy` 表、`governance_reviewer` 账号、其 `SUPER_ADMIN` 角色绑定，以及 `PRODUCT_CONTRACT_RELEASE_APPLY / PRODUCT_CONTRACT_ROLLBACK / VENDOR_MAPPING_RULE_PUBLISH / VENDOR_MAPPING_RULE_ROLLBACK` 四条固定复核策略。
