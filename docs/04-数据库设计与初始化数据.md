...
- `sql/init.sql`：全量 DDL 基线。脚本会先 `CREATE DATABASE rm_iot`、`USE rm_iot`、`SET NAMES utf8mb4`，再按“先删视图、后删表、再重建”的顺序创建当前基线 `36` 张 MySQL 表，并在末尾补齐 `iot_message_log` 兼容视图。
...
- 历史环境若需对齐当前代码，优先使用最新 `sql/init.sql` / `sql/init-data.sql` / `sql/init-tdengine.sql` 重建或初始化；无法重建时，再人工比对脚本结构或执行 `python scripts/run-real-env-schema-sync.py`。该脚本当前也会补齐 `iot_device.org_id / org_name`、`idx_device_tenant_org_deleted`，并在回填设备归属前拦截“一设备多风险点有效绑定”脏数据。
...
- `/products` 抽屉内当前默认治理流程为“规范证据预设 + 报文证据 compare + 显式 apply”；规范证据、报文证据与 compare/apply 都不新增候选草案表，只有 apply 的 `create / update` 决策才会写入 `iot_product_model`。
- 当前首批规范预设编码为 `landslide-integrated-tilt-accel-crack-v1`，默认围绕“倾角 / 加速度 / 裂缝一体机”产品族执行治理；规范出处、单位、监测内容编码、监测类型编码和原始字段别名当前优先存在 compare 证据快照中，不新增 `iot_product_model_draft`、`iot_product_schema`、`iot_product_model_snapshot` 等平行表。
- 手动样本提炼当前只服务于当前选中的产品，且单次只支持解析 `1` 个设备样本 JSON；`business / status / other` 三类样本都会递归下钻到标量叶子后生成属性候选，同时允许通过 compare 的 `manualDraftItems[]` 补录事件和服务候选。
...
- 当前手动提炼会对 `sampleType=other` 的字段默认标记 `needsReview`；运行期候选仍会对 `codex_verify_*`、`singal_*` 等命名漂移字段标记 `needsReview`，但都不会改动库结构；是否入正式契约完全由治理 apply 接口决定。
...
| 风险处置 | `risk_point`、`risk_point_device`、`iot_alarm_record`、`iot_event_record` | `tenant_id`、`org_id`、业务主键、状态字段 | [02-业务功能与流程说明.md](./02-业务功能与流程说明.md)、[21-业务功能清单与验收标准.md](./21-业务功能清单与验收标准.md) |
...
- 历史库若缺少审计增强列、风险监测绑定表或系统内容治理表，应先按最新 `sql/init.sql` / `sql/init-data.sql` 对齐，必要时执行 `python scripts/run-real-env-schema-sync.py`。
