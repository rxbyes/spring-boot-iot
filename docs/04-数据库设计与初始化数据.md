...
- 当前语义治理支撑仍不新增平行合同表，而是通过旁路支撑表完成：`iot_normative_metric_definition` 维护规范字段，`iot_vendor_metric_evidence` 维护原始字段证据，`iot_product_contract_release_batch` 维护正式发布批次。
- `2026-04-06` 起，控制面第一批读侧直接复用这些既有真相表，不再新增 `contract_release_history`、`risk_metric_catalog_snapshot` 一类平行快照表：正式合同批次列表/详情直接读取 `iot_product_contract_release_batch`，风险指标目录列表/详情直接读取 `risk_metric_catalog`，产品维度覆盖率概览则按 `iot_product_model(property) + risk_metric_catalog + risk_point_device + rule_definition` 聚合。
...
- 设计器首轮继续复用 `iot:products:view` / `iot:products:update` 权限边界，不新增 `sys_menu` 按钮种子；当前初始化基线也无需额外菜单脚本。
...
- 字段：
  ...
  - `threshold_direction`：阈值方向；当前 `value` 默认收口为 `HIGHER_IS_RISKIER`。
