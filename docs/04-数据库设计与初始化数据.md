...
- `status_mirror_strategy`：状态镜像策略，当前首批收口为 `NONE / SENSOR_STATE`；例如父设备 `S1_ZT_1.sensor_state.L1_LF_n`、`S1_ZT_1.sensor_state.L1_SW_n` 都可镜像为子设备 `sensor_state`。共享 `dev` 环境中的深部位移关系当前已按 `L1_SW_n -> LEGACY + SENSOR_STATE` 固化，采集器自身 `S1_ZT_1.temp / S1_ZT_1.humidity / S1_ZT_1.signal_4g` 等父设备状态仍留在父设备产品。
...
- 若历史库仍残留采集器父产品短标识正式字段、对象洞察配置或 resolver snapshot，先执行 `sql/upgrade/20260414_collector_parent_full_path_identifier_migration.sql` 统一把产品合同、风险目录、对象洞察配置与发布快照迁到全路径真相；若同设备 latest 中还存在 `signal_4g` 与 `S1_ZT_1.signal_4g` 这类重复行，再按需执行 `sql/upgrade/20260414_device_property_full_path_dedup.sql` 清理旧短标识 latest。
