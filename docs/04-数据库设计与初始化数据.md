...
- `sql/init.sql`：全量 DDL 基线。脚本会先 `CREATE DATABASE rm_iot`、`USE rm_iot`、`SET NAMES utf8mb4`，再按“先删表、再重建”的顺序创建当前基线 `69` 张 active MySQL 表；当前不再创建 MySQL 兼容视图。
- `sql/init-data.sql`：全量初始化样例数据。主要采用 `INSERT ... ON DUPLICATE KEY UPDATE`、变量查询和幂等删除重建关联数据的方式，适合在同一环境重复执行；当前已显式为验收设备样例写入 `iot_device.org_id / org_name`，保持设备主档与已绑定风险点组织口径一致；`2026-04-25` 起菜单与按钮权限基线统一覆盖五个一级工作台、分组首页、产品详情隐藏路由、业务验收结果页、自动化治理台、无代码接入台、设备命令下发、系统基础资料、风险点与风险策略等页面和按钮权限，并按 `role_code + menu_code` 动态重建 `sys_role_menu`。
...
- 仓库当前不再保留承载正式基线的独立增量 SQL 文件。原增量脚本中已进入正式交付基线的 DDL、索引和样例数据，已经并回 MySQL 初始化文件或收口到 `sql/init-tdengine.sql`；如需补充历史数据清理，只允许提供不改变全量基线的一次性脚本，例如 `sql/upgrade/20260414_device_property_full_path_dedup.sql`。
- 当前目录中最常见的语句模式包括：
  - `DROP TABLE IF EXISTS`：保证新库和重建库可重复执行。
  - `CREATE TABLE` / `CREATE INDEX`：直接建立当前正式基线结构与检索索引。
  - 当前基线不再使用 `CREATE OR REPLACE VIEW` 创建 MySQL 兼容视图；历史旧对象由 schema sync 迁移或删除。
  - `INSERT ... ON DUPLICATE KEY UPDATE`：幂等写入菜单、角色、样例数据和治理种子。
  - 静态审计：`node scripts/audit-menu-permission-seed.mjs` 会从前端路由、工作台配置、UI/后端权限常量与 `sql/init-data.sql` 交叉检查页面路由、隐藏详情页和按钮权限，输出 `coveredRouteMenus / missingRoutePaths / missingPermissionCodes / softDeletedPermissionCodes / unguardedActionButtonCandidates / failures`；其中 `coveredRouteMenus` 用于确认 `无代码接入台` 这类页面已被菜单种子发现，`unguardedActionButtonCandidates` 已覆盖 `StandardButton / StandardActionLink / 原生 button` 以及动态按钮文案，已排除刷新、复制、列设置、抽屉关闭、收藏参数等本地或只读动作，当前验收目标固定为 `0`，若再次出现候选项会进入 `failures`，需补按钮权限或在脚本中登记明确的本地 UI 例外。
...
- `schema/mysql/*.json`、`schema/tdengine/telemetry-domain.json` 是当前数据库结构、中文注释、生命周期、关系与业务边界的唯一真相源；当前没有保留 active MySQL 视图 registry。
...
- 投影：为查询优化的派生对象，例如 `iot_device_metric_latest`。
...
- 快照表、投影表与 runtime manifest 不允许脱离主真相源自行长字段；若确需扩展，必须同时说明来源主表、更新血缘与读写边界。
...
- `sys_business_event_log` 承接业务事件字典第一期真相，当前覆盖登录、非 GET HTTP 写动作、调度异常、设备上行失败，以及正式合同发布/回滚、映射规则发布/回滚、协议治理发布/回滚、风险指标目录发布、设备命令下发完成、接入案例验收启动完成与业务验收运行启动完成等可检索里程碑事件。
- `sys_observability_span_log` 承接轻量调用片段证据，当前覆盖 HTTP 请求、设备上行 `message-flow` 摘要、调度任务异常和慢 SQL 摘要。
...
- `2026-04-25` 起，分组首页和隐藏详情路由也进入页面级菜单真相：`iot-access:overview / risk:ops-overview / risk:config-overview / system:governance-overview / system:quality-workbench-overview` 分别对应 `/device-access`、`/risk-disposal`、`/risk-config`、`/system-management`、`/quality-workbench`；`iot:products:detail-*` 承接产品五段详情页；`system:business-acceptance` 与 `system:business-acceptance:result` 承接业务验收台及结果页；`system:automation-governance` 承接 `/automation-governance`；`system:future-lab` 默认隐藏在侧边栏，作为预研入口。质量工场已不再保留 `/automation-assets`、`/automation-test` 等兼容页面种子。
- 字段：
...
- 作用：业务事件日志表，承接“业务事件字典”的第一期持久化证据，用于按 `trace_id / event_code / object_type / evidence_type` 复盘关键业务动作。
- 字段：
  - `id` / `tenant_id`：事件主键和租户范围。
  - `trace_id`：关联链路追踪号。
  - `event_code` / `event_name` / `domain_code` / `action_code`：业务事件字典编码、展示名、领域和动作。
  - `object_type` / `object_id` / `object_name`：业务对象定位信息。
  - `actor_user_id` / `actor_name`：触发事件的用户。
  - `result_status` / `error_code` / `error_message`：事件结果与失败摘要。
  - `source_type` / `evidence_type` / `evidence_id`：事件来源与证据对象，当前会关联审计日志、接口请求或调度任务。
  - `request_method` / `request_uri` / `duration_ms`：接口类事件的请求和耗时摘要。
  - `metadata_json` / `occurred_at` / `create_time` / `deleted`：扩展上下文、发生时间、入库时间和软删除标记。

##### `sys_observability_span_log`

- 作用：可观测调用片段日志表，承接轻量性能与排障证据，用于在 Redis 时间线、诊断文件和审计日志之外保留可检索摘要。
- 字段：
  - `id` / `tenant_id`：调用片段主键和租户范围。
  - `trace_id` / `parent_span_id`：链路追踪号和父调用片段标识。
  - `span_type` / `span_name`：调用片段类型和名称，当前覆盖 `HTTP_REQUEST / MESSAGE_FLOW / SCHEDULED_TASK / SLOW_SQL`。
  - `domain_code` / `event_code` / `object_type` / `object_id`：领域、事件和业务对象定位。
  - `transport_type`：传输类型，例如 `HTTP / MQTT / SQL / SCHEDULER`。
  - `status` / `duration_ms` / `started_at` / `finished_at`：执行状态、耗时和起止时间。
  - `error_class` / `error_message`：失败时的异常类和脱敏摘要。
  - `tags_json` / `create_time` / `deleted`：低基数标签、入库时间和软删除标记。
...
- `iot_normative_metric_definition` 当前承载首批规范字段库，已内置 `phase1-crack` 场景下的 `value / sensor_state`、`phase2-gnss` 场景下的 `gpsInitial / gpsTotalX / gpsTotalY / gpsTotalZ / sensor_state`，以及 `phase3-deep-displacement` 场景下的 `dispsX / dispsY / sensor_state`，用于 compare 元信息与风险可发布性判定。`2026-04-10` 起，南方激光测距产品 `nf-monitor-laser-rangefinder-v1` 复用 `phase1-crack` 规范字段，但产品对外显示名不再直接沿用裂缝文案；`2026-04-11` 起，深部位移场景中仅 `dispsX / dispsY` 允许进入风险闭环，`sensor_state` 继续保留治理语义。`2026-04-24` 起，编码兜底种子覆盖 `L3_QW / L3_YL / L3_DB / L4_NW / L4_LD`；`2026-04-25` 起继续按附件规范扩展 `L1_ZD / L2_SF / L3_CJ / L3_QY / L4_BMLS`，这些新增字段默认 `risk_enabled=0`，只用于自动识别、中文名/单位展示与后续人工治理，不直接进入风险闭环。同日起，`scripts/run-real-env-schema-sync.py` 写入该表前会检查规范字段 seed 的 `id` 唯一性和活动兜底键 `monitor_content_code / monitor_type_code / identifier` 唯一性，冲突时停止同步并输出明细，避免同一上报编码被刷成多条 active 候选。`2026-04-08` 起，`sql/init-data.sql` 已直接为首批裂缝/GNSS 种子写入 `metric_dimension / threshold_type / semantic_direction / gis_enabled / insight_enabled / analytics_enabled / status / version_no` 基线值，`2026-04-11` 起 `scripts/run-real-env-schema-sync.py` 也会为历史共享 `dev` 环境补齐深部位移规范种子，避免旧库仍停留在 `phase1-crack + phase2-gnss`。
- `2026-04-25` 起，`iot_normative_metric_definition` 新增应用内轻量导入治理链路：`/api/device/normative-metrics/import/preview` 负责预检，`/api/device/normative-metrics/import/apply` 负责落库。导入前会复用与同步脚本一致的兜底键唯一性思路，按 `monitor_content_code / monitor_type_code / identifier` 拦截 active 定义冲突；该能力不新增表结构，不改写 `iot_product_model`，也不直接产生合同发布批次或风险指标目录。
...
- `iot_message_log` 作为消息日志唯一物理热表，承接链路追踪、验收证据和治理回放读写；`2026-04-25` 起，超过热表保留期的数据会先归档到 `iot_message_log_archive`，再按批次删除热表行。
- `iot_message_log_archive` 承接已过热保留期的设备消息原始证据，不参与默认业务读链路，只服务治理审计、人工导出和批次复核。
- `iot_message_log_archive_batch` 承接每次消息日志冷归档治理的批次真相，记录确认报告、候选行数、归档行数、删除行数和失败原因，供共享环境清理前后核账。
...
- 设备资产中心当前的“未登记上报名单”不新增独立资产表：主名单仍来自 `iot_device`，未登记优先读取 `iot_device_invalid_report_state(reason_code=DEVICE_NOT_FOUND,resolved=0)` 的最新态，再回退到 `iot_device_access_error_log` 最近一条失败样本，并按设备补齐 `iot_message_log(message_type=dispatch_failed)` 的最近失败轨迹；若共享环境缺表或缺列，则整体回退到 `dispatch_failed` 来源。
...
- `phase1-crack` 裂缝/南方激光测距与 `phase2-gnss` GNSS 治理当前都会在 compare 阶段补齐规范元信息：命中规范字段库的 `property` 行会返回 `normativeIdentifier / normativeName / riskReady / rawIdentifiers[]`，其中 `riskReady=true` 表示该字段后续可被发布到风险指标目录；当前 `phase1-crack` 只对 `value` 开放，GNSS 只对 `gpsTotalX / gpsTotalY / gpsTotalZ` 开放。南方激光测距产品虽然复用 `phase1-crack` canonical 字段，但 compare/正式字段中文名/对象洞察对外展示统一使用 `激光测距值 / 传感器状态`。`2026-04-25` 起，暂未配置专用产品场景的样本若命中活动规范库中的 `L1_ZD / L2_SF / L3_CJ / L3_QY / L4_BMLS`，同样可返回规范中文名、单位和三态匹配说明；这些新增种子默认不发布风险目录。
...
说明：
- 以下说明覆盖 `sql/init.sql` 中的 IoT MySQL 表，以及 `iot_device_metric_latest` latest 投影表。
...
- 后续从附件规范补充新设备类型时，优先通过 `/products/:productId/mapping-rules -> 规范字段库导入` 执行“预检 -> 无冲突落库”。该入口写入的仍是规范字段库种子，正式产品字段仍需回到 `contracts` 页按样本 compare/apply 与审批发布形成。
...
- 字段：
  ...
  - `create_time`：日志入库时间。
- 运行语义：当前作为设备消息日志唯一热表保留最近可检索原始证据；可观测日志治理 `apply` 只会删除已先行归档到 `iot_message_log_archive` 的过期行，并同步写入 `iot_message_log_archive_batch` 批次台账。
- 退场说明：旧对象名 `iot_device_message_log` 已不再进入 registry、初始化脚本、schema sync manifest 或运行时 bootstrap；历史共享库中若仍存在同名表或视图，由 `python scripts/run-real-env-schema-sync.py` 迁移旧表数据后删除，或直接删除旧兼容视图。`2026-04-26` 起，schema sync 在完成旧表迁移/合并后还会继续按 registry 修正 `iot_message_log` 的表注释与字段注释，避免治理审计长期保留旧英文注释漂移。

##### `iot_message_log_archive`

- 作用：设备消息日志冷归档表，保存超过热表保留期但仍需留痕的原始消息证据。
- 字段：
  - `id` / `original_log_id` / `archive_batch_id`：归档行主键、原热表日志 ID 和归档批次 ID。
  - `tenant_id` / `device_id` / `product_id`：归档消息归属租户、设备和产品。
  - `message_type` / `topic` / `payload`：原始消息类型、接入主题和原始载荷。
  - `report_time` / `trace_id` / `device_code` / `product_key`：排障与回放检索字段。
  - `create_time` / `archived_at`：热表原始入库时间和归档落库时间。
- 运行语义：只作为治理审计、批次核账和人工导出真相表，不进入 `/api/device/message-trace/page` 或默认业务分页查询。

##### `iot_message_log_archive_batch`

- 作用：设备消息日志冷归档批次表，记录一次确认后的 archive-then-delete 执行结果。
- 字段：
  - `id` / `batch_no` / `source_table`：批次主键、批次号和来源热表。
  - `governance_mode` / `status`：执行模式和批次状态。
  - `retention_days` / `cutoff_at`：本次治理保留天数和热表截止时间。
  - `confirm_report_path` / `confirm_report_generated_at` / `confirmed_expired_rows`：人工确认所依据的 dry-run 报告路径、生成时间和确认过期行数。
  - `candidate_rows` / `archived_rows` / `deleted_rows`：候选、归档和删除统计。
  - `failed_reason` / `artifacts_json`：失败原因与扩展证据。
  - `create_time` / `update_time`：批次创建与更新时间。
- 运行语义：与 `logs/observability/observability-log-governance-*.json/.md` 一起构成“确认报告 -> 归档批次 -> 删除结果”的可检索证据链。
...
- 主命名与物理真相：`iot_message_log`。
- 退场对象：`iot_device_message_log` 不再作为表、视图或读取入口保留。

结论：
- 文档、新功能、运行态写入和治理读侧统一使用 `iot_message_log`。
- 历史环境若仍存在 `iot_device_message_log`，只允许在 schema sync 迁移 / 删除旧对象的上下文中出现，不再作为业务依赖扩散。
...
- 菜单审计：提交前可执行 `node scripts/audit-menu-permission-seed.mjs`，确认前端路由、工作台入口、按钮权限码和 `sys_menu` 初始化行保持一致；审计输出会列出已覆盖页面菜单，并把未挂权限的真实写动作归入 `unguardedActionButtonCandidates`。当前脚本已覆盖共享按钮、原生按钮和动态文案按钮，并过滤本地刷新、复制、列设置、抽屉关闭、收藏参数等无需角色授权的动作，该候选清单应保持为空，且出现候选项时会让审计失败。`2026-04-25` 起，后端关键写接口也按同一批按钮权限补齐服务端守卫：产品 CRUD、设备批量转正/密钥轮换/父子关系维护、用户/角色/菜单、站内消息/帮助文档、审计日志删除、告警确认/抑制/关闭、事件派发/关闭、风险点正式绑定与待治理转正均会在服务端校验对应 `GovernancePermissionCodes`。
...
- `2026-04-18` 起，`sql/init-data.sql` 还会继续为 `/device-onboarding` 回填 `无代码接入台` 菜单与管理/运维/开发角色默认授权；`2026-04-25` 起进一步补齐创建/编辑接入案例、模板包维护、批量创建、批量套用模板、触发验收和刷新状态等按钮权限，并由前端按钮与后端写接口共同校验。若共享环境只对齐了 `sql/init.sql` 但未补跑初始化数据，路由虽可手输访问，分组首页、菜单和按钮授权仍不会自动出现。
...
- `2026-04-25` 起，五个分组首页直接落页面菜单：`/device-access`、`/risk-disposal`、`/risk-config`、`/system-management`、`/quality-workbench`。产品详情五段隐藏路由、业务验收结果页、自动化治理台和 `/future-lab` 也进入初始化数据；`/future-lab` 默认不在侧边栏展示。
- `对象洞察台` 已挂入 `风险运营`
- 质量工场当前只保留 `业务验收台` 与 `自动化治理台` 两条主路径；旧 `研发工场 / 自动化工场 / 自动化资产中心 / 执行中心 / 结果与基线中心` 均不再作为页面菜单种子保留。
...
- 同日起，`sql/init-data.sql` 继续为当前真实业务页补齐隐藏权限节点：接入侧新增 `/protocol-governance`、`/products`、`/devices`、`/reporting`、`/message-trace` 的工作区/回放/详情权限，风险与平台治理侧新增 `risk-point / alarm-center / event-disposal / organization / region / dict / channel / in-app-message / governance-task / governance-ops / governance-approval / business-acceptance` 以及质量工场资产编排、执行配置、结果证据对应的按钮与功能点；`2026-04-25` 起又补齐 `iot:device-capability:view/execute`、`risk:risk-point-binding:execute/approve`、`risk:risk-point-pending-promotion:execute/approve`、`risk:linkage-plan:approve`、无代码接入台 7 个按钮权限、协议治理解密档案发布/回滚细分权限、质量工场结果证据按钮权限、计划编排编辑权限、页面盘点人工补录权限、审计日志删除权限，以及组织、区域、字典、通知渠道、风险点和风险策略页的增删改、导出、测试等按钮权限。前端对应按钮已挂 `v-permission` 或行级 `permission`，对象洞察属性快照中的正式字段治理入口、运行态名称/单位治理入口以及风险点正式绑定抽屉的新增/解绑/替换/删除原生按钮也纳入同一角色矩阵；无代码接入、协议治理、产品和设备主档、设备命令下发、设备密钥轮换、设备父子关系维护、系统基础资料、用户/角色/菜单、站内消息/帮助文档、审计日志删除、告警处置、事件派发/关闭、风险点与正式绑定治理、治理任务台和治理运维台的关键写接口也会做后端权限守卫。
- 上述页面、隐藏详情路由和按钮权限统一由 `scripts/audit-menu-permission-seed.mjs` 审计。隐藏详情路由仍用 `sys_menu.type=1` 表达真实可路由页面并设置 `hiddenInSidebar=true`，按钮或内嵌工作区继续使用 `sys_menu.type=2 / menu_type=2`；`sys_role_menu` 授权按 `menu_code` 解析，业务/运维/开发显式列出边界，管理角色按已授权父页面继承按钮。脚本当前验证口径要求 `missingRoutePaths / missingPermissionCodes / softDeletedPermissionCodes / unguardedActionButtonCandidates / failures` 均为空。
- `设备资产中心` 当前补齐了八个按钮权限菜单：`iot:devices:add`、`iot:devices:update`、`iot:devices:delete`、`iot:devices:export`、`iot:devices:import`、`iot:devices:replace`、`iot:device-capability:view`、`iot:device-capability:execute`
...
- 原独立增量 SQL 目录中的正式基线 DDL、索引和样例数据，已经并回 MySQL 初始化文件或收口到 `sql/init-tdengine.sql`；当前仅额外保留 `sql/upgrade/20260414_device_property_full_path_dedup.sql` 作为历史数据清理例外，不计入正式初始化基线。
- `iot_message_log` 物理真表、`iot_device_metric_latest`、`iot_device_online_session`、`iot_device_access_error_log`、`iot_device_invalid_report_state`、`iot_device_onboarding_case`、`iot_onboarding_template_pack` 等当前正式交付结构，都已直接进入 `sql/init.sql`。
...
- 无法重建时：以最新 `sql/init.sql` 为准人工比对表、列和索引，或执行 `python scripts/run-real-env-schema-sync.py` 做 MySQL schema 对齐。
...
- `python scripts/run-real-env-schema-sync.py` 当前除补列、补索引外，也会清理消息日志历史旧对象：若旧库存在 `iot_device_message_log` 物理表，会迁移 / 合并到 `iot_message_log` 后删除；若旧库存在同名视图，会直接删除。脚本还会在 `iot_device` 维度执行设备机构治理补齐：先检测“一设备多风险点有效绑定”冲突，再仅对 `org_id IS NULL` 且存在有效风险点绑定、且风险点 `org_id / org_name` 完整的设备做回填。
...
erDiagram
    ...
    IOT_DEVICE ||--o{ IOT_MESSAGE_LOG : logs
...
| 设备资产 | `iot_device` + `iot_device_access_error_log` + `iot_message_log`（未登记名单按设备补齐 `dispatch_failed`） | `device_code`、`product_id`、`online_status`、`last_report_time`、`failure_stage`、`trace_id` | 本文 `2.2` 与 [02-业务功能与流程说明.md](./02-业务功能与流程说明.md) |
| 运行日志 | `iot_message_log`（唯一物理热表）、`iot_message_log_archive`、`iot_message_log_archive_batch`、`sys_business_event_log`、`sys_observability_span_log` | `trace_id`、`topic`、`message_type`、`event_code`、`span_type`、`tenant_id`、`batch_no` | [03-接口规范与接口清单.md](./03-接口规范与接口清单.md)、[11-可观测性、日志追踪与消息通知治理.md](./11-可观测性、日志追踪与消息通知治理.md) |
...
7. `iot_device` 1:N `iot_message_log`。
