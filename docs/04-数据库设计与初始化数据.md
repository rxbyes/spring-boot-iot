...
- 约束：
  ...
  - 运行语义：
    - 发布监听器会在 `ProductContractReleasedEvent` 到达后优先 upsert 当前批次快照；后续 `PublishedProductContractSnapshotServiceImpl`、风险指标目录发布以及对象洞察/history/latest 读链都优先读取该表，再退回正式合同字段推导与 legacy fallback。
