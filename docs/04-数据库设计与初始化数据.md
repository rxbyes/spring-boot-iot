...
- `对象洞察台` 已挂入 `风险运营`
- `自动化工场` 已挂入 `质量工场`
- `设备资产中心` 当前补齐了四个按钮权限菜单：`iot:devices:add`、`iot:devices:update`、`iot:devices:delete`、`iot:devices:export`
- 角色授权基线按 `role_code` 回填，避免历史环境中固定角色主键不一致导致菜单授权漂移
- 默认角色授权中，业务人员具备设备资产中心查看/导出，管理/运维/开发/超管具备设备资产中心增删改导出
...
9. `20260319_phase5_device_asset_center.sql`：补齐设备资产中心按钮菜单（新增/编辑/删除/导出）与角色授权，适用于五工作台已落地但 `/devices` 按钮权限仍缺失的历史库。
10. `20260319_phase5_demo_text_repair.sql`：修复历史共享库中演示账号、角色名称和新增按钮菜单标题被写成 `????` 的问题；脚本使用 `SET NAMES utf8mb4` 与 `CONVERT(0x... USING utf8mb4)` 幂等回写中文，规避 Windows 终端编码污染。
