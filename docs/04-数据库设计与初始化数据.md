...
- 设备资产中心当前的“未登记上报名单”不新增独立资产表：主名单仍来自 `iot_device`，未登记只按最近失败归档做只读拼接展示，默认取 `iot_device_access_error_log` 最近一条失败记录；若共享环境缺表，则回退到 `iot_device_message_log(message_type=dispatch_failed)`。
...
20. `20260324_phase5_tdengine_mapping_gap_draft.sql`：`south_rtu / south_multi_displacement / south_gnss_monitor / south_deep_displacement` 四类 live 产品的 TDengine 映射治理草案。
    - 当前脚本默认 `@apply_changes = 0`，只预览 `iot_product_model` property 基线缺口、代表性 live identifier 和建议 seed，不直接改真实库。
    - 当前脚本只会为 `specs_json.tdengineLegacy` 写入 `enabled=false + mappingStatus=PENDING_TDENGINE_VERIFICATION` 的占位结构，不会把未经实库核验的 `stable/column` 直接落成正式映射。
    - 真正执行前，必须先恢复 TDengine REST / SQL 核验能力，并逐 stable 执行 `DESCRIBE` 确认列名；否则只能把该脚本视为治理台账，而不是最终升级 SQL。
...
| 设备资产 | `iot_device` + `iot_device_access_error_log`（缺表时回退 `iot_message_log` / `iot_device_message_log`） | `device_code`、`product_id`、`online_status`、`last_report_time`、`failure_stage`、`trace_id` | 本文 `2.2` 与 [02-业务功能与流程说明.md](./02-业务功能与流程说明.md) |
