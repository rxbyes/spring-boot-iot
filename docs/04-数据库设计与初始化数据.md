...
- `iot_normative_metric_definition` 当前承载首批规范字段库，已内置 `phase1-crack` 场景下的 `value / sensor_state`，以及 `phase2-gnss` 场景下的 `gpsInitial / gpsTotalX / gpsTotalY / gpsTotalZ / sensor_state`，用于 compare 元信息与风险可发布性判定。`2026-04-10` 起，南方激光测距产品 `nf-monitor-laser-rangefinder-v1` 也复用 `phase1-crack` 规范字段，但产品对外显示名不再直接沿用裂缝文案。`2026-04-08` 起，该表还显式补齐 `metric_dimension / threshold_type / semantic_direction / gis_enabled / insight_enabled / analytics_enabled / status / version_no`，避免桥层语义继续只藏在 `metadata_json`。
