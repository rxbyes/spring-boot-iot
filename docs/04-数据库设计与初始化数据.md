...
3. `iot_product` 1:N `iot_product_model`。
4. `iot_product` 1:N `iot_device`。
5. `iot_device` 1:N `iot_device_message_log`。
6. `iot_device` 1:N `iot_device_property`（按标识唯一）。
7. `risk_point` N:N `iot_device`（通过 `risk_point_device` 并细化到测点）。
8. `iot_alarm_record` -> `iot_event_record` -> `iot_event_work_order` 构成告警到处置闭环。
