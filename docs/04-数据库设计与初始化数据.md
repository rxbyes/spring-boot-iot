...
- 时序表初始化：`spring-boot-iot-telemetry` 会在 TDengine `slave_1` 数据源启动阶段自动补齐 `iot_device_telemetry_point` 兼容表和 telemetry v2 raw stable（`iot_raw_measure_point`、`iot_raw_status_point`、`iot_raw_event_point`）；启动阶段只负责 `CREATE IF NOT EXISTS` 级别 schema 补齐，不自动回灌历史数据。真实环境 legacy stable 继续由环境侧治理，代码不负责补 stable DDL。
