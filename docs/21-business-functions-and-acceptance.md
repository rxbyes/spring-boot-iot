...
12. 前端导出模板基线：支持默认/运维/管理导出模板，快速切换导出列配置
13. 前端自定义导出模板基线：支持保存模板与删除模板，模板按页面本地持久化
14. 前端模板共享基线：支持模板 JSON 导入/导出，实现跨账号或跨环境复用
15. 前端模板管理增强基线：支持模板重命名与分组标签筛选
16. 前端模板效率增强基线：支持模板搜索、最近使用置顶，并展示最近使用时间与最后修改时间
17. 前端模板导入预览明细基线：支持在导入确认前查看新增/覆盖/重命名/跳过完整列表，并支持在明细弹窗直接确认导入
...
5. 区域、字典可配置且可被业务模块引用
6. 通知渠道可查询，审计日志支持 HTTP 自动采集、统一返回体失败识别与 MQTT 等后台异常事件入库并可查询
7. 审计日志详情可查看请求参数与响应摘要，敏感字段需自动脱敏，超长内容应有截断标记；列表与详情链路中的雪花 `id` 应保持字符串语义，避免前端因精度丢失打开空详情
...
| 运维人员 | `OPS_STAFF` | 设备接入、设备运维、接入回放、数据校验、告警/事件/风险点 | 面向设备接入与运行维护 |
| 开发人员 | `DEVELOPER_STAFF` | 设备接入全链路 + 规则/预案 + 风险监测增强页 | 面向联调、验证、缺陷定位 |
...
| 93003007 | 93000003 | 审计日志 | `system:audit` | `/audit-log` |
| 93003008 | 93000003 | 菜单管理 | `system:menu` | `/menu` |

说明：
1. 按钮权限同样落在 `sys_menu`，类型为 `type=2`（如 `system:user:add`、`system:role:update`），通过 `sys_role_menu` 绑定到角色。
...
| 角色权限 | `/role` | 新增、编辑、删除、菜单授权、按用户查角色 | `/api/role/*`、`/api/menu/tree` | `sys_role`、`sys_user_role`、`sys_menu`、`sys_role_menu` | 角色主数据、菜单关系与用户角色关系一致 |
| 菜单管理 | `/menu` | 菜单树查看、新增、编辑、删除 | `/api/menu/tree`、`/api/menu/list`、`/api/menu/{id}`、`/api/menu/add`、`/api/menu/update`、`/api/menu/{id}` | `sys_menu`、`sys_role_menu` | 菜单主数据、父子层级与角色引用约束一致 |
...
| 审计日志 | `/audit-log` | 自动采集接口访问审计、后台异常事件查询、列表查询、分页查询、详情查看、删除 | `/api/system/audit-log/*`（其中 `/page` 返回 `PageResult`） | `sys_audit_log` | 执行系统 API 后应新增日志；MQTT 异常时应新增 `system_error` 记录，页面查询结果与表数据一致 |
...
| 系统管理 | 组织、用户、角色、菜单、区域、字典、通知渠道、审计日志 | 已交付 | 基础 CRUD/列表/树形/查询能力与代码映射一致 | API + 页面回归 |
...
| 菜单管理 | `/menu` / `MenuView.vue` | `spring-boot-iot-ui/src/api/menu.ts` | `/api/menu/tree`；`/api/menu/list`；`/api/menu/{id}`；`/api/menu/add`；`/api/menu/update`；`DELETE /api/menu/{id}` | `sys_menu`、`sys_role_menu` | 已接真实后端；支持菜单树查看与菜单 CRUD，删除时校验子菜单与角色引用 |
...
| 审计日志 | `/audit-log` / `AuditLogView.vue` | `spring-boot-iot-ui/src/api/auditLog.ts` | `/api/system/audit-log/list`；`/api/system/audit-log/page`；`/api/system/audit-log/get/{id}`；`/api/system/audit-log/delete/{id}` | `sys_audit_log` | 已接真实后端；详情支持请求/响应摘要展示（脱敏+截断），并支持查看 `system_error` 后台异常记录 |
...
- 角色管理页可加载完整菜单树；用户/角色页面操作按钮受 `menu_code` 权限控制
...
| 审计日志 | `/audit-log` | `/api/system/audit-log/*` | `sys_audit_log` | 通过 |
