...
|---|---|---|---|---|
| 业务人员 | `BUSINESS_STAFF` | 风险运营工作台：实时监测台、GIS态势图、告警运营台、事件协同台、对象洞察台、运营分析中心 | `/risk-disposal` | 面向日常业务监测、处置与复盘 |
| 管理人员 | `MANAGEMENT_STAFF` | 风险运营 + 风险策略 + 平台治理主体能力 + 账号/角色部分按钮权限 | `/risk-disposal` | 面向业务统筹、规则审批、组织治理 |
| 运维人员 | `OPS_STAFF` | 接入智维全链路 + 风险运营精选页 + 风险策略中的风险对象/阈值配置 | `/device-access` | 面向设备接入、异常排障与运行维护 |
| 开发人员 | `DEVELOPER_STAFF` | 接入智维全链路 + 风险运营排障页 + 风险策略全链路 + 质量工场 | `/device-access` | 面向联调、验证、缺陷定位 |
...
3. 账号中心、角色权限、导航编排页的新增 / 编辑 / 删除 / 重置密码按钮统一通过 `v-permission` 按 `authContext.permissions` 收口；导航编排页“前往角色授权”入口额外要求 `system:role:update`。
