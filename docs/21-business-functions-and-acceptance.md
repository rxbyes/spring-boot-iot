...
5. 区域、字典可配置且可被业务模块引用
6. 通知渠道可查询，审计日志支持自动采集并可查询
7. 审计日志详情可查看请求参数与响应摘要，敏感字段需自动脱敏，超长内容应有截断标记
...
| 审计日志 | `/audit-log` | 自动采集接口访问审计、列表查询、分页查询、详情查看、删除 | `/api/system/audit-log/*`（其中 `/page` 返回 `PageResult`） | `sys_audit_log` | 执行系统 API 后应新增日志，页面查询结果与表数据一致 |
...
| 审计日志 | `/audit-log` / `AuditLogView.vue` | `spring-boot-iot-ui/src/api/auditLog.ts` | `/api/system/audit-log/list`；`/api/system/audit-log/page`；`/api/system/audit-log/get/{id}`；`/api/system/audit-log/delete/{id}` | `sys_audit_log` | 已接真实后端；详情支持请求/响应摘要展示（脱敏+截断） |
