...
2. 明确未交付：风险监测实时监测与 GIS 地图、首页驾驶舱增强、设备中心增强
...
4. 区域管理：`/api/region`
5. 字典配置：`/api/dict`
6. 通知渠道：`/system/channel`
7. 审计日志：`/system/audit-log`
...
2. 启动通过：`mvn -pl spring-boot-iot-admin spring-boot:run -Dspring-boot.run.profiles=dev`
...
1. 后端统一以 `application-dev.yml` 启动：`mvn -pl spring-boot-iot-admin spring-boot:run -Dspring-boot.run.profiles=dev`
...
| 风险点管理 | `/risk-point` | 新增、编辑、删除、绑定设备、解绑设备、查看绑定设备 | `/api/risk-point/*` | `risk_point`、`risk_point_device` | 风险点主数据与绑定关系一致 |
...
| 分析报表 | `/report-analysis` | 打开页面、切换时间范围、发起四类查询 | `/api/report/risk-trend`、`/api/report/alarm-statistics`、`/api/report/event-closure`、`/api/report/device-health` | 理论应聚合 `iot_alarm_record`、`iot_event_record`、`iot_device`、`iot_message_log` | 当前以接口联通与页面不报错为主，统计准确性待后端补全 |
...
| 通知渠道 | `/channel` | 新增、编辑、删除、按编码查询 | `/system/channel/*` | `sys_notification_channel` | 渠道主数据可增删改查 |
| 审计日志 | `/audit-log` | 列表查询、分页查询、详情查看、删除 | `/system/audit-log/*` | `sys_audit_log` | 页面查询结果与表数据一致 |
...
当前限制：
1. `ReportServiceImpl` 仍为 `TODO`，接口返回更多用于连通性验证，不宜作为统计准确性最终收口依据。
2. 本轮可验收“接口可访问、页面不报错、参数可传递”，不把统计结果正确性作为通过项。
...
| 风险平台增强 | 实时监测、GIS 地图、驾驶舱增强 | 未交付 | 当前不纳入本轮验收，后续按 `docs/19-phase4-progress.md` 滚动补齐 | 不纳入 |
...
| 风险点管理 | `/risk-point` / `RiskPointView.vue` | `spring-boot-iot-ui/src/api/riskPoint.ts` | `/api/risk-point/list`；`/api/risk-point/get/{id}`；`/api/risk-point/add`；`/api/risk-point/update`；`/api/risk-point/delete/{id}`；`/api/risk-point/bind-device`；`/api/risk-point/unbind-device`；`/api/risk-point/bound-devices/{riskPointId}` | `risk_point`、`risk_point_device`、`iot_device` | 已按后端真实路径对齐前端调用 |
...
| 通知渠道 | `/channel` / `ChannelView.vue` | `spring-boot-iot-ui/src/api/channel.ts` | `/system/channel/list`；`/system/channel/getByCode/{channelCode}`；`/system/channel/add`；`/system/channel/update`；`/system/channel/delete/{id}` | `sys_notification_channel` | 已接真实后端 |
| 审计日志 | `/audit-log` / `AuditLogView.vue` | `spring-boot-iot-ui/src/api/auditLog.ts` | `/system/audit-log/list`；`/system/audit-log/page`；`/system/audit-log/get/{id}`；`/system/audit-log/delete/{id}` | `sys_audit_log` | 已接真实后端 |
...
| 报表分析 | `/report-analysis` / `ReportAnalysisView.vue` | `spring-boot-iot-ui/src/api/report.ts` | `/api/report/risk-trend`；`/api/report/alarm-statistics`；`/api/report/event-closure`；`/api/report/device-health` | 规划应聚合 `iot_alarm_record`、`iot_event_record`、`iot_device`、`iot_message_log`、`iot_device_property` | 前端已接接口，但 `ReportServiceImpl` 当前仍是 `TODO`，返回结果未真正落库统计 |
| 风险监测增强 | `/risk-monitoring` / `RealTimeMonitoringView.vue`；`/risk-monitoring-gis` / `RiskGisView.vue` | `spring-boot-iot-ui/src/api/riskMonitoring.ts` | `/api/risk-monitoring/realtime/list`；`/api/risk-monitoring/realtime/{bindingId}`；`/api/risk-monitoring/gis/points` | `risk_point`、`risk_point_device`、`iot_alarm_record`、`iot_event_record`、`iot_device`、`iot_device_property`、`iot_message_log`、`iot_product` | 前后端页面、接口与详情抽屉已落到当前代码基线；但按 `docs/19-phase4-progress.md` 仍不计入本轮已交付范围，GIS 仅为 ECharts 点位态势图 |
