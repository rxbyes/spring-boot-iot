...
10. 前端列表交互基线：组织、用户、角色、区域、字典、通知渠道、业务日志，以及告警中心、事件处置均统一支持“已选项计数 / 清空选中 / 刷新列表”操作栏
11. 前端列表导出基线：上述页面均支持“导出选中”与“导出当前结果”CSV（本地导出，不依赖新增后端接口），并包含中文列头与关键状态文案映射
12. 前端导出设置基线：支持“导出列设置”（列勾选 + 顺序调整），导出配置按页面本地持久化
13. 前端导出模板基线：支持默认/运维/管理导出模板，快速切换导出列配置
14. 前端自定义导出模板基线：支持保存模板与删除模板，模板按页面本地持久化
15. 前端模板共享基线：支持模板 JSON 导入/导出，实现跨账号或跨环境复用
16. 前端模板管理增强基线：支持模板重命名与分组标签筛选
17. 前端模板效率增强基线：支持模板搜索、最近使用置顶，并展示最近使用时间与最后修改时间
18. 前端模板导入预览明细基线：支持在导入确认前查看新增/覆盖/重命名/跳过完整列表，并支持在明细弹窗直接确认导入
19. 前端详情交互基线：实时监测 / GIS、告警中心、事件处置、业务日志、系统日志统一使用右侧详情抽屉；抽屉需支持统一标题区、状态标签、分组信息展示，以及加载 / 空态 / 错误态反馈
20. 前端表单交互基线：用户、角色、菜单、组织、区域、字典、通知渠道，以及风险点、阈值规则、联动规则、应急预案的新增 / 编辑统一使用右侧表单抽屉；风险点绑定设备、事件处置的派发 / 关闭、字典项管理及其新增 / 编辑也统一使用抽屉；抽屉需保留现有表单校验、提交按钮与关闭重置逻辑
...
3. 登录接口与 `/api/auth/me` 返回 `authContext`，前端据此恢复角色信息与按钮权限；公共壳层导航优先由 `authContext.menus` 动态构建
...
| 业务日志 | `/audit-log` | 自动采集接口访问审计、列表查询、分页查询、详情查看、删除 | `/api/system/audit-log/*`（其中业务日志页默认传 `excludeSystemError=true`） | `sys_audit_log` | 执行系统 API 后应新增业务操作审计，页面默认不混入 `system_error` 后台异常；详情统一走右侧抽屉 |
| 系统日志 | `/system-log` | 后台异常事件查询、列表查询、分页查询、详情查看、删除 | `/api/system/audit-log/*`（其中系统日志页固定传 `operationType=system_error`） | `sys_audit_log` | MQTT / 后台异步异常时应新增 `system_error` 记录，页面查询结果与表数据一致；详情统一走右侧抽屉 |
...
| 自动化测试 | `/automation-test` / `AutomationTestCenterView.vue` | 无（前端本地编排，导出 JSON 计划） | 无（由 `scripts/auto/run-browser-acceptance.mjs --plan=...` 读取计划执行） | `localStorage`、`config/automation/*.json`、`logs/acceptance/*` | 已交付配置驱动骨架；支持模板编排、变量捕获、报告导出与测试建议生成 |
...
| 业务日志 | `/audit-log` / `AuditLogView.vue` | `spring-boot-iot-ui/src/api/auditLog.ts` | `/api/system/audit-log/list`；`/api/system/audit-log/page`；`/api/system/audit-log/get/{id}`；`/api/system/audit-log/delete/{id}` | `sys_audit_log` | 已接真实后端；默认排除 `system_error`，详情支持请求/响应摘要展示（脱敏+截断） |
| 系统日志 | `/system-log` / `AuditLogView.vue` | `spring-boot-iot-ui/src/api/auditLog.ts` | `/api/system/audit-log/list`；`/api/system/audit-log/page`；`/api/system/audit-log/get/{id}`；`/api/system/audit-log/delete/{id}` | `sys_audit_log` | 已接真实后端；固定查看 `system_error` 后台异常，支持按模块、通道、目标 / URL 排查 |
