...
3. 登录接口与 `/api/auth/me` 返回 `authContext`，前端据此恢复角色信息与按钮权限；公共壳层导航采用固定分组模板并保持居中布局
...
| 审计日志 | `/audit-log` | 自动采集接口访问审计、列表查询、分页查询、详情查看、删除 | `/api/system/audit-log/*` | `sys_audit_log` | 执行系统 API 后应新增日志，页面查询结果与表数据一致 |
...
- 若共享开发环境出现 `authContext.menus=[]`，前端壳层会按本文档 5.1 的一级/二级模块矩阵与 `roleCodes` 执行临时菜单兜底，仅用于保障可导航性；数据库动态菜单仍是正式口径
