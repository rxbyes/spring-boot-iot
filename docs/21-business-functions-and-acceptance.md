...
8. 审计中心：`/api/system/audit-log`（前端业务日志页默认排除 `system_error`）
9. 异常观测台：接入智维分区复用 `/api/system/audit-log`，固定查看 `operation_type=system_error`
10. 链路追踪台：`/api/device/message-trace/page`，支持按 `TraceId`、设备编码、产品标识、消息类型、Topic 分页检索接入消息日志
11. 前端列表交互基线：组织架构、账号中心、角色权限、区域版图、数据字典、通知编排、导航编排、审计中心，以及告警运营台、事件协同台、风险对象中心、阈值策略、联动编排、应急预案库、实时监测台、GIS态势图统一采用“KPI 概览卡 + 筛选卡 + 列表卡 / 资源卡”工作台结构；其中列表页支持“已选项计数 / 清空选中 / 刷新列表”等操作栏，平台治理列表默认对超长单元格内容执行单行省略并在悬停时展示完整值，不再在列表内自动换行，GIS 未定位对象使用资源卡收拢展示
...
20. 前端详情交互基线：实时监测台 / GIS态势图、告警运营台、事件协同台、审计中心、异常观测台统一使用右侧详情抽屉；抽屉需支持统一标题区、状态标签、分组信息展示，以及加载 / 空态 / 错误态反馈；其中审计中心、异常观测台、链路追踪台详情默认采用“概览卡片 + 链路信息 + 深色报文块 + 结果提示卡”的日志型展示结构，告警运营台、事件协同台、风险监测详情默认采用“概览卡片 + 业务分区 + 说明提示卡”的业务型展示结构，且概览卡、字段卡、提示卡、最近记录卡统一保持浅色控制台式轻卡片风格
21. 前端表单交互基线：账号中心、角色权限、导航编排、组织架构、区域版图、数据字典、通知编排，以及风险对象中心、阈值策略、联动编排、应急预案库的新增 / 编辑统一使用右侧表单抽屉；风险对象中心绑定设备、事件协同台的派发 / 关闭、字典项管理及其新增 / 编辑也统一使用抽屉；自动化工场的导入计划 / 新增自定义页面、公共导出列设置及其模板命名 / 导入冲突处理、账号中心 / 修改密码等工具型交互也统一收口为抽屉；抽屉需保留现有表单校验、提交按钮与关闭重置逻辑，其中风险对象中心、阈值策略、联动编排、应急预案库的抽屉内部统一采用“提示卡 + 分区卡 + 栅格字段”的表单内容结构
22. 前端分页与配色基线：2026-03-18 起告警运营台、事件协同台、风险对象中心、阈值策略、联动编排、应急预案库，以及组织架构、账号中心、角色权限、导航编排、区域版图、数据字典、通知编排、审计中心、异常观测台、链路追踪台、实时监测台统一使用 `StandardPagination` + `useServerPagination` 分页契约（其中告警运营台、事件协同台当前沿用后端 `/list` + 前端本地切片分页）；同时 `tokens.css` 与 `element-overrides.css` 的主色统一收敛到 `--brand`，壳层导航与头部高亮由 token 变量驱动
23. 前端公共壳层基线：2026-03-18 起公共壳层统一为“品牌 / 全局搜索 / 工具区 + 一级业务分区条 + 左侧分组摘要与二级菜单 + 内容页标准页头”的浅色控制台结构；内容页头需提供面包屑、主标题、页面说明、状态卡以及返回分组概览入口，避免导航层级与内容顶层表达不一致
...
6. 通知编排可查询，审计中心支持 HTTP 自动采集，异常观测台支持 MQTT 等后台异常事件入库并可查询
...
8. 异常观测台支持按 `TraceId`、设备编码、产品标识、异常编码、异常类型排查，并可直接跳转链路追踪台
9. 链路追踪台支持按 `TraceId` / Topic / 设备编码检索 `iot_message_log`，并可回跳异常观测台完成双向联查
10. 审计中心与异常观测台均支持按 `operationResult`（成功/失败）筛选，并在页面展示统计概览：业务侧聚合总量/今日/成功失败/活跃用户，系统侧聚合异常总量/今日/MQTT/链路数/高频模块
...
4. 前端一级/二级导航优先使用登录返回的 `authContext.menus` 动态渲染；仅在菜单树为空时启用临时静态兜底。
5. 账号中心、角色权限、导航编排页的新增 / 编辑 / 删除 / 重置密码按钮统一通过 `v-permission` 按 `authContext.permissions` 收口；导航编排页“前往角色授权”入口额外要求 `system:role:update`。
