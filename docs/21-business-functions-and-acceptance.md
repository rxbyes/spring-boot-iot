...
6. `GET /api/device/page`，分页查询设备资产台账
7. `PUT /api/device/{id}`，更新设备资产
8. `DELETE /api/device/{id}`，删除单台设备
9. `POST /api/device/batch-delete`，批量删除设备
10. `POST /message/http/report`，HTTP 模拟上报入口
11. `GET /api/device/{deviceCode}/properties`，查询最新属性
12. `GET /api/device/{deviceCode}/message-logs`，查询消息日志
...
2. 当前未计入已交付：风险监测实时监测与 GIS 地图已进入代码基线但待真实环境验收；首页已完成商业化重构并进一步升级为“数据看板驾驶舱 + 事务工作台入口”双层结构，支持一线/运维/管理/研发四类角色视角，但当前仍采用静态能力编排与本地 activity 记录，不纳入真实环境验收；设备资产中心已交付首个可用闭环，但批量导入、设备更换、远程维护、维修工单联动仍待后续迭代
...
|---|---|---|---|---|
| 业务人员 | `BUSINESS_STAFF` | 风险运营工作台 + 设备资产中心查看/导出 | `/risk-disposal` | 面向日常业务监测、处置复盘与库存核查 |
| 管理人员 | `MANAGEMENT_STAFF` | 风险运营 + 风险策略 + 平台治理主体能力 + 设备资产中心增删改导出 + 账号/角色部分按钮权限 | `/risk-disposal` | 面向业务统筹、规则审批、组织治理 |
| 运维人员 | `OPS_STAFF` | 接入智维全链路 + 风险运营精选页 + 风险策略中的风险对象/阈值配置 + 设备资产中心增删改导出 | `/device-access` | 面向设备接入、异常排障与运行维护 |
| 开发人员 | `DEVELOPER_STAFF` | 接入智维全链路 + 风险运营排障页 + 风险策略全链路 + 质量工场 + 设备资产中心增删改导出 | `/device-access` | 面向联调、验证、缺陷定位 |
...
2. 设备资产中心当前新增按钮权限种子：`93001101 iot:devices:add`、`93001102 iot:devices:update`、`93001103 iot:devices:delete`、`93001104 iot:devices:export`。
3. 前端一级/二级导航优先使用登录返回的 `authContext.menus` 动态渲染；仅在菜单树为空时启用临时静态兜底。
4. 账号中心、角色权限、导航编排页的新增 / 编辑 / 删除 / 重置密码按钮统一通过 `v-permission` 按 `authContext.permissions` 收口；导航编排页“前往角色授权”入口额外要求 `system:role:update`。
...
| IoT 基础 | `/devices` / `DeviceWorkbenchView.vue` | `spring-boot-iot-ui/src/api/device.ts` | `POST /api/device/add`；`GET /api/device/{id}`；`GET /api/device/code/{deviceCode}`；`GET /api/device/page`；`PUT /api/device/{id}`；`DELETE /api/device/{id}`；`POST /api/device/batch-delete` | `iot_device` | 已接真实后端，当前以“设备资产中心”承接设备台账、详情、增改删、批量删除与本地 CSV 导出 |
| 链路验证中心 | `/reporting` / `ReportWorkbenchView.vue` | `spring-boot-iot-ui/src/api/iot.ts` | `POST /message/http/report` | `iot_message_log`、`iot_device_property`、`iot_device` | 已接真实后端，当前以前端“链路验证中心”方式承接 HTTP 上报验证主链路 |
