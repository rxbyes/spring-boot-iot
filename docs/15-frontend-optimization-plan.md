...
- 本轮收口结论：
  ...
  - 普通产品台账页的主操作应放在台账卡片头部，与标题形成稳定的“标题 + 主按钮”结构；不再把“新增”按钮悬挂在 Hero 说明区，避免首屏视觉重心漂移。
...
- 本轮落地结果：
  - `ProductWorkbenchView` 已移除冗余总数展示、展开筛选说明和全部库存口径。
  - 页面筛选已收口为“产品名称 / 节点类型 / 产品状态”三项同列查询。
  - “新增产品”已迁移到台账卡片头部，首屏只保留简洁定位和非库存指标。
...
- **产品定义中心标准化**：`/products` 已从联调型产品工作台重构为“紧凑概览卡 + 常用筛选/更多条件 + 列表卡 + 详情抽屉 + 表单抽屉”的标准产品台账页，统一接入 `StandardTableToolbar`、`StandardPagination`、`StandardDetailDrawer`、`StandardFormDrawer`、`StandardDrawerFooter` 与 CSV 导出列设置；首屏 KPI 已切换到共享 `MetricCard` 紧凑规格，筛选区已形成“默认只看常用条件、其余条件按需展开”的试点样式，后续新增产品物模型、批量导入等能力时不得回退到页面私有表单和请求面板
