...
- **头部体验**：顶部右侧补齐控制台工具区（工作台 + 消息/帮助入口），头像区展示当前登录账号与角色，增强全局信息可见性
...
- **ID 精度治理**：前端 API 层已引入 `IdType = string | number` 并统一替换 `id/*Id` 主键类型；后端 `Long` 主键按字符串返回，避免 JS 精度丢失导致详情/关闭等操作错位
