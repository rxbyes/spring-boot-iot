...
- **风险平台分页组件统一**：2026-03-18 起，告警中心、事件处置、风险点管理、阈值规则、联动规则、应急预案统一切换为共享 `StandardPagination`；`useServerPagination` 新增 `setTotal` 与 `applyLocalRecords`，用于在“后端真分页”和“本地分页切片”两类列表中复用同一分页契约
