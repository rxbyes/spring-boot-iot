...
- 公共壳层采用“顶部双层导航 + 左侧业务菜单 + 右侧内容区”的控制台结构，参考阿里云控制台的信息组织方式
- 公共壳层视觉已切换为浅色云控制台风格，业务页面仍保持现有组件体系与联调能力
- 当前已经接入 `Element Plus` 组件体系和 `ECharts` 图表能力

## 页面结构

### 公共壳层（2026-03）
- 顶部第一行：品牌入口、页面搜索、快捷入口、连接状态
- 顶部第二行：一级业务分区导航（概览 / IoT 基础 / 风险处置 / 系统管理）
- 左侧：当前一级分区下的二级菜单，显示页面名称与用途说明
- 右侧：工作区标题、API Base URL 运行时切换、登录态操作、最近访问标签和业务内容
- 移动端：侧边菜单切换为抽屉式展开

### 视觉一致性优化（2026-03）
- 全局 Token、Element Plus 组件覆盖和通用卡片样式已切换为浅色控制台体系
- 已优先完成驾驶舱、产品工作台、设备工作台、上报工作台、风险点工作台等核心页面的深色面板替换
...
**商业化改造（Phase 3）**：
- 角色切换：支持一线人员、运维人员、管理人员三种角色视图切换
- 实时风险趋势图：集成 ECharts 展示近 24 小时风险趋势
- 风险构成饼图：展示红橙黄蓝四色风险分布
- 预警处置看板：展示待处置、处置中、已处置、超时预警的进度
- 角色快捷入口：每个角色提供 4 个快捷导航卡片
- 精简文字：将原文字精简至 30%，提升可视化占比
...
- 为避免商业化首页点击无反馈，驾驶舱 `/` 当前不要求登录；其他受保护页面在未登录时会回退到驾驶舱
