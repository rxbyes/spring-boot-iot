...
- 若是历史库或自建旧环境首次联调风险监测，额外确认已执行 `sql/upgrade/20260316_phase4_task3_risk_monitoring_schema_sync.sql`
...
- 风险监测实时监测、GIS 风险态势（2026-03-24 已在当前共享开发环境完成 `/api/risk-monitoring/realtime/list`、`/api/risk-monitoring/realtime/{bindingId}`、`/api/risk-monitoring/gis/points` 以及 `/risk-monitoring`、`/risk-monitoring-gis` 真实环境复验；历史旧环境首次联调前仍需确认已执行 `20260316_phase4_task3_risk_monitoring_schema_sync.sql`）
