...
3. 若大量接口返回 `500`，优先检查真实库 schema 是否与当前代码一致；必要时先按最新 `sql/init.sql` / `sql/init-data.sql` 对齐，或执行 `python scripts/run-real-env-schema-sync.py` 后再复测。若风险点接口报 `Unknown column 'org_id' in 'field list'`，说明 `risk_point.org_id / org_name` 尚未补齐。
