...
额外约束：
- 若共享 `9999` 已运行其他工作区实例，只允许改端口启动当前验收实例；不得为了抢占真实 `$dp` 流量，直接再起第二个并行消费同一共享 Broker 的 live consumer。
- 若 `GET /actuator/health`、`message-flow` 摘要字段或当前工作区构建结果明显不一致，应先判定为“当前验收入口不是本分支实例”，再决定改用隔离实例、独立 Broker、真实设备窗口或记录环境阻塞。
...
- 若共享 `127.0.0.1:9999` 指向的不是当前分支实例，且现网 `$dp` 仍在持续接入，禁止通过“再起一个并行 consumer”来抢验收；此时应记录为“实例对不齐 / 需隔离 MQTT 验收环境”的阻塞项。
- MQTT 客户端日志无异常
...
### 6.5 分家族验收记录口径
每个家族至少保留 1 条成功样本，并记录以下检查点：
- `message-flow` 固定 10 阶段完整，且 `status=COMPLETED`
- `PROTOCOL_DECODE.summary.familyCodes` 与预期家族一致；加密包同时核对 `appId`
- `PROTOCOL_DECODE.summary.normalizationStrategy` 为 `LEGACY_DP`，或在兼容灰度时明确记录为 `LEGACY_DP_COMPAT`
- `PAYLOAD_APPLY.summary.propertyCount`、`childMessageCount` 与该家族预期一致
- `TELEMETRY_PERSIST.branch`、`legacyMappedMetricCount`、`legacyUnmappedMetricCount`、`fallbackMetricCount`、`fallbackReason` 留痕完整；若命中 `NORMALIZED_FALLBACK_ONLY`，只能判定为“fallback 命中”，不能直接判定为 legacy 治理通过
- `GET /api/telemetry/latest?deviceId=<DEVICE_ID>` 可查到最新值、`storageType=tdengine`，并能关联本次 `traceId`
- TDengine legacy stable 或 `iot_device_telemetry_point` 至少有一侧能查到本次样本；若两侧都缺失，直接判为不通过
- 深部位移基准站家族需额外核对父设备和拆分后的子设备都留下独立时序点

记录建议：
- 每个家族至少保留 `traceId / sessionId / deviceCode / appId / familyCodes / normalizationStrategy / TELEMETRY_PERSIST.branch / fallbackReason`
- 若本次验收入口不是当前分支实例，或共享 Broker 不支持回放 `$dp` 回投，必须把阻塞原因一并写入验收记录
