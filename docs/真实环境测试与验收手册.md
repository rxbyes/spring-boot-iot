...
- 发送成功后，页面应立即展示本次 HTTP 提交对应的“处理时间线”面板，阶段顺序固定为 `INGRESS -> TOPIC_ROUTE -> PROTOCOL_DECODE -> DEVICE_CONTRACT -> MESSAGE_LOG -> PAYLOAD_APPLY -> DEVICE_STATE -> RISK_DISPATCH -> COMPLETE`。
...
- 返回成功后应至少拿到 `sessionId`，并允许 `/reporting` 按该 `sessionId` 轮询处理状态。
...
- `/reporting` 页面先显示“等待消费回流”，随后在同一 `sessionId` 绑定 trace 后展示完整 timeline
