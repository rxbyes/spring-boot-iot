...
- 系统管理：组织、用户、角色、区域、字典、通知渠道、审计日志已完成基础交付。
...
### 2.2.1 ???????????
- 2026-03-16 ?? `spring-boot-iot-admin/src/main/resources/application-dev.yml` ???????????`/api/auth/login` ?????????????????????
- `/api/risk-monitoring/realtime/list`?`/api/risk-monitoring/gis/points` ??????????????????????????? `/api` ?????
- ????????????
  - ?? `risk_point_device` ??
  - `risk_point` ???? Phase 4 ????? `create_by` / `update_by`?
- ??? `sql/upgrade/20260316_phase4_task3_risk_monitoring_schema_sync.sql` ????????????????????????????
...
| P1 | ???? | ?????? API ?? | ?????????? | ?????? | ??? `20260316_phase4_task3_risk_monitoring_schema_sync.sql` |
| P1 | ???? | ?????? API ?? | ?????????? | ?????? | ? 24h ???????????? |
| P1 | ???? | ???????? | ??????????????? | `RealTimeMonitoringView` | ??????????????? |
| P1 | GIS | ??? GIS API ?? | ?????????? | GIS ???? | ??? `20260316_phase4_task3_risk_monitoring_schema_sync.sql` |
| P1 | GIS | GIS ???? | ??????????????? | `RiskGisView` | ?????????????????? |
| P2 | 驾驶舱 | KPI 与趋势图模块增强 | 未开始 | CockpitView 增强 | KPI 与趋势显示 |
| P2 | 驾驶舱 | 最新告警与待办事件 | 未开始 | 列表模块 | 数据可钻取 |
...
| 2026-03-16 | ???????????????????? `risk_point_device` ? `risk_point` ??????? `20260316_phase4_task3_risk_monitoring_schema_sync.sql` ????????? | alarm / SQL / docs | Codex | ??????????? `/api/risk-monitoring/*` |
