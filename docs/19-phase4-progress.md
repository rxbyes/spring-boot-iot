...
- 风险监测前端页面：
  - `RealTimeMonitoringView` 已完成开发，待真实环境联调。
  - `RiskGisView` 已完成开发，待真实环境联调。
...
| P1 | 风险监测 | 实时监测列表 API 联调 | 已开发待验证 | 列表查询结果 | 支持筛选与分页 |
| P1 | 风险监测 | 实时监测详情 API 联调 | 已开发待验证 | 详情查询结果 | 含 24h 趋势、告警摘要、事件摘要 |
| P1 | 风险监测 | 实时监测前端页面 | 已完成开发待真实环境验证 | `RealTimeMonitoringView` | 列表、筛选、分页、详情抽屉可用 |
| P1 | GIS | 风险点 GIS API 联调 | 已开发待验证 | GIS 点位数据 | 支持区域筛选与无经纬度兜底 |
| P1 | GIS | GIS 前端页面 | 已完成开发待真实环境验证 | `RiskGisView` | 点位态势图、详情抽屉、未定位列表可用 |
...
- 后端运行：`mvn -pl spring-boot-iot-admin spring-boot:run -Dspring-boot.run.profiles=dev`
