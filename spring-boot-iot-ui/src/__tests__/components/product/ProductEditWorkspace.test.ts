...
describe('ProductEditWorkspace', () => {
  it('renders the flattened edit workspace and emits cancel and submit actions', async () => {
    ...
    expect(wrapper.find('.product-edit-workspace__chapter-header').exists()).toBe(true)
    ...
    expect(wrapper.find('.product-edit-workspace__chapter-brief').text()).toContain('当前变更会直接回写')
    ...
    expect(wrapper.find('.product-edit-workspace__form-intro').text()).toContain('按基础档案')
