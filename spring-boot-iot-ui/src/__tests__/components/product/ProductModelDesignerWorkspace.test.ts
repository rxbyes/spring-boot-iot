...
describe('ProductModelDesignerWorkspace', () => {
  ...
  it('loads manual extraction guidance in a 2026 exhibition board with curation strips and no briefing ledger', async () => {
    ...
    expect(wrapper.find('.product-model-designer__exhibit-head').exists()).toBe(true)
    ...
    expect(wrapper.find('.product-model-designer__curation-strip').exists()).toBe(true)
    expect(wrapper.findAll('.product-model-designer__curation-strip-item')).toHaveLength(3)
    ...
    expect(wrapper.find('.product-model-designer__curation-flow').exists()).toBe(true)
