...
import { canAccessSectionHome, createSectionHomeNavItem, getSectionHomeConfigByPath, matchSectionCardActivity, pickSectionActivities, resolveSectionHomeConfig } from '@/config/sectionHomes';
...
describe('sectionHomes config', () => {
  it('resolves section home config by group key and label', () => {
    ...
    expect(resolveSectionHomeConfig('risk-core', '风险处置')?.navLabel).toBe('处置概览');
  });
  ...
  it('grants section home access when any child path is authorized', () => {
    ...
    expect(getSectionHomeConfigByPath('/risk-disposal')?.title).toBe('风险处置');
  });
