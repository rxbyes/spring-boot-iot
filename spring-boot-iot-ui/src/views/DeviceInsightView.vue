  ...
  <div class="risk-workbench-page">
    ...
    <div class="workbench-header">
      <div class="header-left">
        <h1 class="page-title">监测对象工作台</h1>
    ...
    <div class="main-workarea">
      ...
      <div class="role-quick-access">
        <h3 class="section-title">业务导航</h3>
...
const roleActions = {
  field: [
    { icon: '??', title: '监测对象工作台', path: '/insight' },
  ...
  ops: [
    ...
    { icon: '??', title: '阈值管理', path: '/devices' },
    { icon: '??', title: '设备巡检', path: '/devices' },
    { icon: '??', title: '数据完整性校验', path: '/file-debug' }
  ],
  manager: [
...
async function refreshAll() {
  ...
  try {
    ...
    ElMessage.success(`监测对象 ${deviceCode.value} 刷新成功`);
    ...
    recordActivity({
      ...
      ok: true,
      detail: `监测对象 ${deviceCode.value} 刷新完成，属性 ${propertyResponse.data.length} 条，日志 ${logResponse.data.length} 条`
    });
  } catch (error) {
