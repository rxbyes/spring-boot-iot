<template>
  <div class="risk-gis-view">
    <PanelCard
      ...
      title="GIS 风险态势"
      description="本轮 GIS 交付限定为 ECharts 经纬度点位态势图，不包含第三方地图 SDK 或底图能力。"
    >
    ...
    <PanelCard
      ...
      title="风险点位态势图"
      :description="`已定位 ${locatedPoints.length} 个点位，未定位 ${unlocatedPoints.length} 个点位。颜色映射风险等级，大小映射活跃告警数。`"
    >
...
function renderChart() {
  ...
  const data = locatedPoints.value.map((point) => ({
    name: point.riskPointName || `风险点 ${point.riskPointId}`,
