  ...
  <div class="report-workbench-page">
    ...
    <div class="workbench-header">
      <div class="header-left">
        <h1 class="page-title">接入验证中心</h1>
        <span class="timestamp">{{ currentTime }}</span>
      </div>
      <div class="header-right">
        <el-radio-group v-model="currentRole" size="large">
          <el-radio-button value="field">实施验证</el-radio-button>
          <el-radio-button value="ops">运维核验</el-radio-button>
        </el-radio-group>
      </div>
    </div>
    ...
    <div class="main-workarea">
      ...
      <div class="report-config">
        <h3 class="section-title">接入验证配置</h3>
...
const lastResponse = ref<unknown>({ tip: '发起验证后，这里会出现统一响应体。' });
...
const reportSummary = computed<ReportSummary>(() => {
  if (!reportForm.deviceCode || !reportForm.productKey) {
    return {
      ...
      shortLabel: 'NA',
      tone: 'blue',
      description: '请先配置设备编码和产品 Key，准备发起验证。'
    };
  }
  ...
  let label = '蓝色验证';
  let shortLabel = '蓝';
  let description = '当前验证配置完整，可进行模拟接入验证。';

  if (score >= 40) {
    tone = 'yellow';
    label = '黄色验证';
    shortLabel = '黄';
    description = '当前验证配置存在部分缺失，建议补充关键字段后再继续验证。';
  } else if (score >= 15) {
    tone = 'orange';
    label = '橙色验证';
    shortLabel = '橙';
    description = '当前验证配置需要重点关注，建议补充协议和格式配置。';
  }
...
const roleActions = {
  field: [
    ...
    { icon: '??', title: '监测对象工作台', desc: '查看监测对象的属性与日志线索', path: '/insight' },
    { icon: '??', title: '属性趋势查看', desc: '分析监测对象的历史趋势', path: '/insight' },
    { icon: '??', title: '风险复盘材料', desc: '补充处置前后的分析记录', path: '/insight' }
  ],
  ops: [
    { icon: '??', title: '阈值管理', desc: '参数配置与远程调整', path: '/devices' },
    { icon: '??', title: '设备巡检', desc: '离线与弱信号设备', path: '/devices' },
    { icon: '??', title: '数据完整性校验', desc: '文件与固件聚合结果核验', path: '/file-debug' },
    { icon: '??', title: '接入验证中心', desc: '模拟设备上报与链路验证', path: '/reporting' }
  ]
};
...
async function handleSendReport() {
  ...
  try {
    ...
    ElMessage.success(`设备 ${reportForm.deviceCode} 接入验证成功`);
    recordActivity({
      module: '接入验证中心',
      action: '发送接入验证',
