  ...
  >
    <template #actions>
      <StandardActionGroup gap="sm">
        <StandardButton v-permission="'system:automation-governance:evidence:import-json'" action="confirm" @click="handleImport">导入 JSON</StandardButton>
        <StandardButton v-permission="'system:automation-governance:evidence:clear-import'" action="reset" :disabled="!importedRun" @click="$emit('clear')">
          清空结果
        </StandardButton>
      </StandardActionGroup>
    </template>
